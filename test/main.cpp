...
int main(int argc, char **argv)
{
    ...
    PetscCall(PetscOptionsGetInt(NULL, NULL, "-refine_levels", &refineLevels, NULL));
...
externalForce.setFz(fz_func);
    ...
    auto safe_rate = [](PetscReal eCoarse, PetscReal eFine) -> PetscReal {
        if (eCoarse > 0.0 && eFine > 0.0) return std::log(eCoarse / eFine) / std::log(2.0);
