...
// 帮助信息
static char help[] = "测试 DUAL_MAC 求解器\n"
                     "选项：\n"
                     "  -nx <n>          : x 方向网格点数 (默认: 16)\n"
                     "  -ny <n>          : y 方向网格点数 (默认: 16)\n"
                     "  -nz <n>          : z 方向网格点数 (默认: 16)\n"
...
int main(int argc, char **argv)
{
    ...
    // 固定三组网格级别：N = 4, 6, 8（对应 Nx=Ny=Nz）
    const std::vector<PetscInt> gridLevels = {4, 6, 8};
    PetscReal nu = 1;           // 动力粘度
    PetscReal tfinal = 1;     // 最终时间
...
// 
// 示例1：零函数（当前设置）
RefSol::ScalarFunc ux_func = [](PetscScalar x, PetscScalar y, PetscScalar z, PetscScalar t) -> PetscScalar {
    return (2.0-t)*std::cos(2*M_PI*z);
};
RefSol::ScalarFunc uy_func = [](PetscScalar x, PetscScalar y, PetscScalar z, PetscScalar t) -> PetscScalar {
    return (1.0+t)*std::sin(2*M_PI*z);
};
RefSol::ScalarFunc uz_func = [](PetscScalar x, PetscScalar y, PetscScalar z, PetscScalar t) -> PetscScalar {
    return (1.0-t)*std::sin(2*M_PI*x);
};

RefSol::ScalarFunc omegax_func = [](PetscScalar x, PetscScalar y, PetscScalar z, PetscScalar t) -> PetscScalar {
    return -2.0*M_PI*std::cos(2*M_PI*z)*(t + 1);
};
    RefSol::ScalarFunc omegay_func = [](PetscScalar x, PetscScalar y, PetscScalar z, PetscScalar t) -> PetscScalar {
    return 2*M_PI*std::cos(2*M_PI*x)*(t - 1) + 2*M_PI*std::sin(2*M_PI*z)*(t - 2);
};
RefSol::ScalarFunc omegaz_func = [](PetscScalar x, PetscScalar y, PetscScalar z, PetscScalar t) -> PetscScalar {
    return 0.0;
};
RefSol::ScalarFunc p_func = [](PetscScalar x, PetscScalar y, PetscScalar z, PetscScalar t) -> PetscScalar {
    auto pi = M_PI;
    return sin(2*pi*(t + x + y)) + (cos(2*pi*z)*cos(2*pi*z)*(t - 2)*(t - 2))/2 + (sin(2*pi*x)*sin(2*pi*x)*(t - 1)*(t - 1))/2 + (sin(2*pi*z)*sin(2*pi*z)*(t + 1)*(t + 1))/2-((3*t*t)/4 - t + 1.5);
};
// 示例2：自定义函数（取消注释并修改）
// RefSol::ScalarFunc ux_func = std::function<PetscScalar(PetscScalar, PetscScalar, PetscScalar, PetscScalar)>(
//     +[](PetscScalar x, PetscScalar y, PetscScalar z, PetscScalar t) -> PetscScalar {
...
// TODO: 用户实现外部力
// 示例：
// externalForce.setFx([](PetscScalar x, PetscScalar y, PetscScalar z, PetscScalar t) { return ...; });
// externalForce.setFy([](PetscScalar x, PetscScalar y, PetscScalar z, PetscScalar t) { return ...; });
// externalForce.setFz([](PetscScalar x, PetscScalar y, PetscScalar z, PetscScalar t) { return ...; });

ExternalForce::ForceFunc fx_func = [](PetscScalar x, PetscScalar y, PetscScalar z, PetscScalar t) -> PetscScalar {
    auto pi = M_PI;
    return 2*pi*cos(2*pi*(t + x + y)) - cos(2*pi*z) - sin(2*pi*x)*(2*pi*cos(2*pi*x)*(t - 1) + 2*pi*sin(2*pi*z)*(t - 2))*(t - 1) - 4*pi*pi*cos(2*pi*z)*(t - 2) + 2*pi*cos(2*pi*x)*sin(2*pi*x)*(t - 1)*(t - 1);

};
ExternalForce::ForceFunc fy_func = [](PetscScalar x, PetscScalar y, PetscScalar z, PetscScalar t) -> PetscScalar {
    auto pi = M_PI;
    return sin(2*pi*z) + 2*pi*cos(2*pi*(t + x + y)) + 4*pi*pi*sin(2*pi*z)*(t + 1) - 2*pi*cos(2*pi*z)*sin(2*pi*x)*(t - 1)*(t + 1);
};
ExternalForce::ForceFunc fz_func = [](PetscScalar x, PetscScalar y, PetscScalar z, PetscScalar t) -> PetscScalar {
    auto pi = M_PI;
    return cos(2*pi*z)*(2*pi*cos(2*pi*x)*(t - 1) + 2*pi*sin(2*pi*z)*(t - 2))*(t - 2) - sin(2*pi*x) - 4*pi*pi*sin(2*pi*x)*(t - 1) - 2*pi*cos(2*pi*z)*sin(2*pi*z)*(t - 2)*(t - 2);
};
...
externalForce.setFz(fz_func);
    ...
    auto safe_rate = [](PetscReal eCoarse, PetscReal eFine, PetscReal hRatio) -> PetscReal {
        if (eCoarse > 0.0 && eFine > 0.0 && hRatio > 1.0) {
            return std::log(eCoarse / eFine) / std::log(hRatio);
        }
        return std::numeric_limits<PetscReal>::quiet_NaN();
    };

    // ===== 6. 固定网格级别循环：N=4,6,8 =====
    for (size_t level = 0; level < gridLevels.size(); ++level) {
        const PetscInt Nx = gridLevels[level];
        const PetscInt Ny = gridLevels[level];
        const PetscInt Nz = gridLevels[level];
        // 令 Nt 与 Nx 近似同比例增长，保持时间分辨率随网格加密同步提升
        const PetscInt Nt = PetscMax((PetscInt)1, (PetscInt)std::lround((double)baseNt * ((double)Nx / (double)baseNx)));
        ...
        PetscCall(PetscPrintf(PETSC_COMM_WORLD,
                              ...
                              "固定压力: %s\n"
                              "========================================\n\n",
                              (PetscInt)level + 1, (PetscInt)gridLevels.size(),
                              Nx, Ny, Nz, Nt, (double)tfinal, (double)nu,
                              (double)xmin, (double)xmax, (double)ymin, (double)ymax,
                              (double)zmin, (double)zmax,
                              pinPressure ? "是" : "否"));

        // ===== 7. 创建求解器 =====
        DUAL_MAC solver(tfinal, nu, Nx, Ny, Nz, Nt,
        ...
        LevelErrorData row;
        row.level = (PetscInt)level + 1;
        row.Nx = Nx; row.Ny = Ny; row.Nz = Nz; row.Nt = Nt;
    ...
    for (const auto &e : allErrors) {
        PetscCall(PetscPrintf(PETSC_COMM_WORLD,
                              "level=%" PetscInt_FMT ", N=(%" PetscInt_FMT ",%" PetscInt_FMT ",%" PetscInt_FMT "), Nt=%" PetscInt_FMT "\n"
                              "  u1=%.12e, omega2=%.12e, p0=%.12e\n"
                              "  u2=%.12e, omega1=%.12e, p3=%.12e\n",
                              e.level, e.Nx, e.Ny, e.Nz, e.Nt,
                              (double)e.err_u1, (double)e.err_omega2, (double)e.err_p0,
                              (double)e.err_u2, (double)e.err_omega1, (double)e.err_p3));
    }

    if (allErrors.size() >= 2) {
        PetscCall(PetscPrintf(PETSC_COMM_WORLD,
                              "----------------------------------------\n"
                              "相邻层收敛率 r = log(E_c / E_f) / log(h_c / h_f)\n"
                              "----------------------------------------\n"));
        for (size_t i = 1; i < allErrors.size(); ++i) {
            const auto &c = allErrors[i - 1];
            const auto &f = allErrors[i];
            // 在固定物理域下 h ~ 1/N，因此 h_c/h_f = N_f/N_c
            const PetscReal hRatio = (c.Nx > 0) ? ((PetscReal)f.Nx / (PetscReal)c.Nx) : 0.0;
            PetscCall(PetscPrintf(PETSC_COMM_WORLD,
                                  "level %" PetscInt_FMT " -> %" PetscInt_FMT ":\n"
                                  "  u1=%.6f, omega2=%.6f, p0=%.6f\n"
                                  "  u2=%.6f, omega1=%.6f, p3=%.6f\n",
                                  c.level, f.level,
                                  (double)safe_rate(c.err_u1, f.err_u1, hRatio),
                                  (double)safe_rate(c.err_omega2, f.err_omega2, hRatio),
                                  (double)safe_rate(c.err_p0, f.err_p0, hRatio),
                                  (double)safe_rate(c.err_u2, f.err_u2, hRatio),
                                  (double)safe_rate(c.err_omega1, f.err_omega1, hRatio),
                                  (double)safe_rate(c.err_p3, f.err_p3, hRatio)));
        }
    }
