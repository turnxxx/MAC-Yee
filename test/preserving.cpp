...
int main(int argc, char **argv) {
  ...
  RefSol::ScalarFunc omegax_func = [](PetscScalar x, PetscScalar y,
                                      ...
                                      PetscScalar t) -> PetscScalar {
    auto pi = M_PI;
    return -2 * pi * cos(2 * pi * z);
  };
  RefSol::ScalarFunc omegay_func = [](PetscScalar x, PetscScalar y,
                                      PetscScalar z,
                                      PetscScalar t) -> PetscScalar {
    auto pi = M_PI;
    return -2 * pi * cos(2 * pi * x) - 2 * pi * sin(2 * pi * z);
  };
  ...
  ExternalForce::ForceFunc fx_func =
      [](PetscScalar x, PetscScalar y, PetscScalar z,
         PetscScalar t) -> PetscScalar { return 0.0; };
  ExternalForce::ForceFunc fy_func =
      [](PetscScalar x, PetscScalar y, PetscScalar z,
         PetscScalar t) -> PetscScalar { return 0.0; };
  ExternalForce::ForceFunc fz_func =
      [](PetscScalar x, PetscScalar y, PetscScalar z,
         PetscScalar t) -> PetscScalar { return 0.0; };
  ...
  auto safe_rate = [](PetscReal eCoarse, PetscReal eFine,
                      PetscReal hRatio) -> PetscReal {
  ...
  for (size_t level = 0; level < gridLevels.size(); ++level) {
    ...
    PetscCall(PetscPrintf(
        ...
        "固定压力: %s\n"
        "========================================\n\n",
        (PetscInt)level + 1, (PetscInt)gridLevels.size(), Nx, Ny, Nz, Nt,
        (double)tfinal, (double)nu, (double)xmin, (double)xmax, (double)ymin,
        (double)ymax, (double)zmin, (double)zmax, pinPressure ? "是" : "否"));
  ...
  for (const auto &e : allErrors) {
    PetscCall(PetscPrintf(
        ...
        (double)e.err_p0, (double)e.err_u2, (double)e.err_omega1,
        (double)e.err_p3));
  }
  ...
  if (allErrors.size() >= 2) {
    ...
    for (size_t i = 1; i < allErrors.size(); ++i) {
      ...
      const PetscReal hRatio =
          (c.Nx > 0) ? ((PetscReal)f.Nx / (PetscReal)c.Nx) : 0.0;
      PetscCall(PetscPrintf(
          PETSC_COMM_WORLD,
          "level %" PetscInt_FMT " -> %" PetscInt_FMT ":\n"
          "  u1=%.6f, omega2=%.6f, p0=%.6f\n"
          "  u2=%.6f, omega1=%.6f, p3=%.6f\n",
          c.level, f.level, (double)safe_rate(c.err_u1, f.err_u1, hRatio),
