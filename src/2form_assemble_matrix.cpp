...
// 2. 耦合方程(5.2)：∇×u₂ - ω₁ = 0
// 3. 散度方程(5.3)：∇·u₂ = 0
PetscErrorCode DUAL_MAC::assemble_2form_system_matrix(
    DM dmSol_1, DM dmSol_2, Mat A, Vec rhs, Vec u2_prev, Vec omega2_known,
    Vec omega1_prev, ExternalForce externalForce, PetscReal time,
    PetscReal dt) {
  ...
  // 注意：assemble_u2_omega1_coupling_matrix 组装的是 ω₁ - ∇×u₂ = 0
  // 所以它已经包含了 +I 项（对 ω₁）
  ...
  // 包含时间导数项、对流项和旋度项：
  // - +u₂^{k-1}/dt（时间导数项）
  // - -0.5 * ω₂^{h,k-1/2} × u₂^{k-1}（对流项，移项后变负）
  // - -0.5/Re * ∇_h × ω₁^{h,k-1}（旋度项，移项后变负）
  DUAL_MAC_DEBUG_LOG("[DEBUG] [2-form] 右端项组装开始\n");
  PetscCall(assemble_rhs2_vector(dmSol_2, rhs, u2_prev, dmSol_1, omega2_known,
                                 omega1_prev, dt));
...
// 2. 对流项：-0.5 * ω₂^{h,k-1/2} × u₂^{k-1}（移项后变负）
// 3. 旋度项：-0.5/Re * ∇_h × ω₁^{h,k-1}（移项后变负）
PetscErrorCode DUAL_MAC::assemble_rhs2_vector(DM dmSol_2, Vec rhs, Vec u2_prev,
  ...
  PetscPrintf(PETSC_COMM_WORLD, "assemble_rhs2_vector: curl_coeff: %g\n",
              curl_coeff);
  ...
  PetscCall(
      DMGlobalToLocal(dmSol_2, omega1_prev, INSERT_VALUES, localOmega1Prev));
  ...
  for (PetscInt ez = startz; ez < startz + nz; ++ez) {
    for (PetscInt ey = starty; ey < starty + ny; ++ey) {
      for (PetscInt ex = startx; ex < startx + nx; ++ex) {
        ...
        PetscScalar omega2_y =
            0.25 *
            (arrOmega2[ez][ey][ex][slot_omega2_y] +     // 本地DOWN面
        ...
        PetscScalar omega2_z =
            0.25 *
            (arrOmega2[ez][ey][ex][slot_omega2_z] +     // 本地BACK面
        ...
        // x 方向面（LEFT）位于 x=prev, y=center, z=center
        // z 方向面（BACK）位于 z=prev，需要在 x 和 z 两个方向上插值
        PetscScalar u2_z_prev =
            0.25 *
            (arrU2Prev[ez][ey][ex][slot_u2_z] +     // 本地BACK面
        ...
        // x 方向面（LEFT）位于 x=prev, y=center, z=center
        // y 方向面（DOWN）位于 y=prev，需要在 x 和 y 两个方向上插值
        PetscScalar u2_y_prev =
            0.25 *
            (arrU2Prev[ez][ey][ex][slot_u2_y] +     // 本地DOWN面
        ...
        PetscCall(DMStagVecSetValuesStencil(dmSol_2, rhs, 1, &row, &rhs_val,
                                            ADD_VALUES));
        ...
        PetscScalar omega2_z_y =
            0.25 * (arrOmega2[ez][ey][ex][slot_omega2_z] +     // 本地BACK面
        ...
        PetscScalar omega2_x =
            0.25 * (arrOmega2[ez][ey][ex][slot_omega2_x] +     // 本地LEFT面
        ...
        // y 方向面（DOWN）位于 y=prev, x=center, z=center
        // x 方向面（LEFT）位于 x=prev，需要在 x 和 y 两个方向上插值
        PetscScalar u2_x_prev =
            0.25 * (arrU2Prev[ez][ey][ex][slot_u2_x] +         // 本地LEFT面
        ...
        // y 方向面（DOWN）位于 y=prev, x=center, z=center
        // z 方向面（BACK）位于 z=prev，需要在 y 和 z 两个方向上插值
        PetscScalar u2_z_prev_y =
            0.25 * (arrU2Prev[ez][ey][ex][slot_u2_z] +         // 本地BACK面
        ...
        PetscScalar omega2_y_z =
            0.25 * (arrOmega2[ez][ey][ex][slot_omega2_y] +     // 本地DOWN面
        ...
        // z 方向面（BACK）位于 z=prev, x=center, y=center
        // y 方向面（DOWN）位于 y=prev，需要在 y 和 z 两个方向上插值
        PetscScalar u2_y_prev_z =
            0.25 * (arrU2Prev[ez][ey][ex][slot_u2_y] +         // 本地DOWN面
  ...
  PetscCall(
      DMStagVecRestoreArrayRead(dmSol_2, localOmega1Prev, &arrOmega1Prev));
...
// 组装u2时间矩阵
PetscErrorCode DUAL_MAC::assemble_u2_dt_matrix(DM dmSol_2, Mat A,
                                               PetscReal dt) {
  PetscFunctionBeginUser;
  PetscInt startx, starty, startz, nx, ny, nz;
  PetscCall(DMStagGetCorners(dmSol_2, &startx, &starty, &startz, &nx, &ny, &nz,
  ...
  for (PetscInt ez = startz; ez < startz + nz; ++ez) {
    for (PetscInt ey = starty; ey < starty + ny; ++ey) {
      for (PetscInt ex = startx; ex < startx + nx; ++ex) {
        ...
        // ---- x 方向面 (LEFT) ----
        row.loc = LEFT;
        PetscCall(DMStagMatSetValuesStencil(dmSol_2, A, 1, &row, 1, &row,
                                            &coeff, ADD_VALUES));
...
PetscErrorCode DUAL_MAC::assemble_u2_conv_matrix(DM dmSol_2, Mat A, DM dmSol_1,
                                                 Vec omega2_known) {
  PetscFunctionBeginUser;
  PetscInt startx, starty, startz, nx, ny, nz;
  PetscCall(DMStagGetCorners(dmSol_2, &startx, &starty, &startz, &nx, &ny, &nz,
...
PetscErrorCode DUAL_MAC::assemble_omega1_curl_matrix(PetscReal Re, DM dmSol_2,
                                                     Mat A) {
  PetscFunctionBeginUser;
  PetscInt startx, starty, startz, nx, ny, nz;
  PetscCall(DMStagGetCorners(dmSol_2, &startx, &starty, &startz, &nx, &ny, &nz,
  ...
  PetscPrintf(PETSC_COMM_WORLD, "assemble_omega1_curl_matrix: coeff: %g\n",
              coeff);
  // 遍历所有单元
  for (PetscInt ez = startz; ez < startz + nz; ++ez) {
    for (PetscInt ey = starty; ey < starty + ny; ++ey) {
      for (PetscInt ex = startx; ex < startx + nx; ++ex) {
        ...
        col[nCol].c = 0;
        val[nCol] = coeff / hz; // 正号来自 -(ω₁ʸ|_{z=next} - ω₁ʸ|_{z=prev}) =
                                // (ω₁ʸ|_{z=prev} - ω₁ʸ|_{z=next}) / hz
        nCol++;
        ...
        col[nCol].c = 0;
        val[nCol] = -coeff / hz; // 负号来自 -(ω₁ʸ|_{z=next} - ω₁ʸ|_{z=prev}) =
                                 // (ω₁ʸ|_{z=prev} - ω₁ʸ|_{z=next}) / hz
        nCol++;
        ...
        col[nCol].c = 0;
        val[nCol] = coeff / hx; // 正号来自 -(ω₁ᶻ|_{x=next} - ω₁ᶻ|_{x=prev}) =
                                // (ω₁ᶻ|_{x=prev} - ω₁ᶻ|_{x=next}) / hx
        nCol++;
        ...
        col[nCol].c = 0;
        val[nCol] = -coeff / hx; // 负号来自 -(ω₁ᶻ|_{x=next} - ω₁ᶻ|_{x=prev}) =
                                 // (ω₁ᶻ|_{x=prev} - ω₁ᶻ|_{x=next}) / hx
        nCol++;
  ...
  for (PetscInt ez = startz; ez < startz + nz; ++ez) {
    for (PetscInt ey = starty; ey < starty + ny; ++ey) {
      for (PetscInt ex = startx; ex < startx + nx; ++ex) {
        ...
        // 左单元中心（x=prev）
        // 本地LEFT面位于
        // x=prev，应使用本地单元中心(ex,...)和左边单元中心(ex-1,...)
        col[0].i = ex - 1;
        col[0].j = ey;
        col[0].k = ez;
        col[0].loc = ELEMENT;
        ...
        PetscCall(DMStagMatSetValuesStencil(dmSol_2, A, 1, &row, 2, col, val,
                                            ADD_VALUES));

        // ===== y 方向面 (DOWN) 的压力梯度项 =====
        // ∂P₃/∂y ≈ (P₃|_{y=center} - P₃|_{y=prev}) / hy
        // DOWN 面位于
        // y=prev，应使用本地单元中心(ex,ey,ez)和下方单元中心(ex,ey-1,ez)
        row.loc = DOWN; // y方向面
        ...
        // BACK 面位于
        // z=prev，应使用本地单元中心(ex,ey,ez)和后方单元中心(ex,ey,ez-1)
        row.loc = BACK; // z方向面
  ...
  for (PetscInt ez = startz; ez < startz + nz; ++ez) {
    for (PetscInt ey = starty; ey < starty + ny; ++ey) {
      for (PetscInt ex = startx; ex < startx + nx; ++ex) {
        ...
        // ===== 添加 +I 项（对 ω₁）=====
        // 对于每个棱，添加 +1 的对角项（ω₁ - ∇×u₂ = 0 的 ω₁ 部分）
        row.loc = BACK_DOWN;
        row.c = 0;
        PetscScalar plus_one = 1.0;
        PetscCall(DMStagMatSetValuesStencil(dmSol_2, A, 1, &row, 1, &row,
                                            &plus_one, ADD_VALUES));
    ...
    for (PetscInt ey = starty; ey < starty + ny; ++ey) {
      for (PetscInt ex = startx; ex < startx + nx; ++ex) {
        ...
        if (this->pinPressure && ex == 0 && ey == 0 && ez == 0) {
          ...
          PetscScalar one = 1.0;
          PetscCall(DMStagMatSetValuesStencil(dmSol_2, A, 1, &row, 1, &pcol,
                                              &one, ADD_VALUES));
          continue;
        }
...
// 组装外力项向量
// 在每一个面上取对应点和方向的外力值
PetscErrorCode DUAL_MAC::assemble_force2_vector(DM dmSol_2, Vec rhs,
  ...
  PetscCall(
      DMStagGetProductCoordinateLocationSlot(dmSol_2, ELEMENT, &icx_center));
  PetscCall(DMStagGetProductCoordinateLocationSlot(dmSol_2, LEFT, &icx_prev));
  PetscCall(
      DMStagGetProductCoordinateLocationSlot(dmSol_2, ELEMENT, &icy_center));
  PetscCall(DMStagGetProductCoordinateLocationSlot(dmSol_2, LEFT, &icy_prev));
  PetscCall(
      DMStagGetProductCoordinateLocationSlot(dmSol_2, ELEMENT, &icz_center));
  ...
  for (PetscInt ez = startz; ez < startz + nz; ++ez) {
    for (PetscInt ey = starty; ey < starty + ny; ++ey) {
      for (PetscInt ex = startx; ex < startx + nx; ++ex) {
        ...
        PetscCall(DMStagVecSetValuesStencil(dmSol_2, rhs, 1, &row, &force_val,
                                            ADD_VALUES));
