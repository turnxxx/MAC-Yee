...
// 2. 耦合方程(5.5)：∇×u₁ - ω₂ = 0
// 3. 散度方程(5.6)：∇·u₁ = 0
PetscErrorCode DUAL_MAC::assemble_1form_system_matrix(DM dmSol_1, DM dmSol_2, Mat A, Vec rhs, Vec u1_prev, Vec omega1_known, Vec omega2_prev, ExternalForce externalForce, PetscReal time, PetscReal dt)
{
    ...
    // 对应方程(5.5)中的 ∇_h × u₁^{h,k+1/2} - ω₂^{h,k+1/2} = 0
    // 注意：assemble_u1_omega2_coupling_matrix 组装的是 ∇×u₁ - ω₂ = 0
    // 所以它已经包含了 -I 项（对 ω₂）
    ...
    // 包含时间导数项、对流项和旋度项：
    // - u₁^{k-1/2}/dt（时间导数项）
    // - 0.5 * ω₁^{h,k} × u₁^{k-1/2}（对流项）
    // - -0.5/Re * ∇_h × ω₂^{h,k-1/2}（旋度项）
    DUAL_MAC_DEBUG_LOG("[DEBUG] [1-form] 右端项组装开始\n");
    PetscCall(assemble_rhs1_vector(dmSol_1, rhs, u1_prev, dmSol_2, omega1_known, omega2_prev, dt));
    ...
    // 将 (0,0,0) 处的 p0 方程固定为 p0 = 0，缓解周期边界下压力零空间导致的慢收敛。
    if (this->pinPressure) {
        ...
        prow.c = 0;
        PetscCall(DMStagVecSetValuesStencil(dmSol_1, rhs, 1, &prow, &pval, INSERT_VALUES));
        }
        PetscCall(VecAssemblyBegin(rhs));
        PetscCall(VecAssemblyEnd(rhs));
    }
...
// 2. 对流项：ω₁^{h,k} × u₁^{h,k-1/2}（上一步的对流项，因为对流项是半隐式的）
PetscErrorCode DUAL_MAC::assemble_rhs1_vector(DM dmSol_1, Vec rhs, Vec u1_prev, DM dmSol_2, Vec omega1_known, Vec omega2_prev, PetscReal dt)
{
    PetscFunctionBeginUser;
    ...
    PetscCall(DMGlobalToLocal(dmSol_1, omega2_prev, INSERT_VALUES, localOmega2Prev));
    PetscCall(DMStagVecGetArrayRead(dmSol_1, localOmega2Prev, &arrOmega2Prev));

    // 获取 slot 索引
    PetscInt slot_u1_x, slot_u1_y, slot_u1_z;
    PetscCall(DMStagGetLocationSlot(dmSol_1, BACK_DOWN, 0, &slot_u1_x));  // x方向棱
    PetscCall(DMStagGetLocationSlot(dmSol_1, BACK_LEFT, 0, &slot_u1_y));  // y方向棱
    PetscCall(DMStagGetLocationSlot(dmSol_1, DOWN_LEFT, 0, &slot_u1_z));  // z方向棱
    ...
    for (PetscInt ez = startz; ez < startz + nz; ++ez) {
      for (PetscInt ey = starty; ey < starty + ny; ++ey) {
        for (PetscInt ex = startx; ex < startx + nx; ++ex) {
          ...
          PetscScalar u1_z_prev = 0.25 * (arrU1Prev[ez][ey][ex][slot_u1_z] +
          ...
          PetscScalar u1_y_prev = 0.25 * (arrU1Prev[ez][ey][ex][slot_u1_y] +
          ...
          curl_x += (arrOmega2Prev[ez][ey][ex][slot_omega2_z] - arrOmega2Prev[ez][ey-1][ex][slot_omega2_z]) / hy;
          // 第二项：-∂ω₂ʸ/∂z ≈ -(ω₂ʸ|_{ez} - ω₂ʸ|_{ez-1}) / hz
          // ω₂ʸ 在 xz 平面上（法向 y），即 DOWN 和 UP 面
          curl_x -= (arrOmega2Prev[ez][ey][ex][slot_omega2_y] - arrOmega2Prev[ez-1][ey][ex][slot_omega2_y]) / hz;
          rhs_val += curl_coeff * curl_x;

          PetscCall(DMStagVecSetValuesStencil(dmSol_1, rhs, 1, &row, &rhs_val, ADD_VALUES));
          ...
          // y 方向棱（BACK_LEFT）位于 x=prev, z=prev
          // 使用四个位置的双线性插值（在 x 和 y 两个方向上）
          PetscScalar omega1_x = 0.25 * (arrOmega1[ez][ey][ex][slot_omega1_x] +          // BACK_DOWN (当前单元)
                                         arrOmega1[ez][ey][ex-1][slot_omega1_x] +        // 左邻居的 BACK_DOWN
                                         arrOmega1[ez][ey+1][ex][slot_omega1_x] +        // BACK_UP (上邻居的BACK_DOWN)
                                         arrOmega1[ez][ey+1][ex-1][slot_omega1_x]);      // 左上邻居的 BACK_DOWN

          // u₁ˣ^{k-1/2}：需要插值到 y 方向棱的位置，使用四个 x 方向棱（在 x 和 y 两个方向上）
          PetscScalar u1_x_prev = 0.25 * (arrU1Prev[ez][ey][ex][slot_u1_x] +          // BACK_DOWN (当前单元)
                                         arrU1Prev[ez][ey][ex-1][slot_u1_x] +        // 左邻居的 BACK_DOWN
                                         arrU1Prev[ez][ey+1][ex][slot_u1_x] +        // BACK_UP (上邻居的BACK_DOWN)
                                         arrU1Prev[ez][ey+1][ex-1][slot_u1_x]);      // 左上邻居的 BACK_DOWN

          // u₁ᶻ^{k-1/2}：需要插值到 y 方向棱的位置，使用四个 z 方向棱
          PetscScalar u1_z_prev_y = 0.25 * (arrU1Prev[ez][ey][ex][slot_u1_z] +
                                            arrU1Prev[ez][ey+1][ex][slot_u1_z] +
                                            arrU1Prev[ez-1][ey][ex][slot_u1_z] +
                                            arrU1Prev[ez-1][ey+1][ex][slot_u1_z]);

          // 对流项：0.5 * (ω₁ᶻ u₁ˣ - ω₁ˣ u₁ᶻ)（0.5 来自时间平均）
          ...
          curl_y += (arrOmega2Prev[ez][ey][ex][slot_omega2_x] - arrOmega2Prev[ez-1][ey][ex][slot_omega2_x]) / hz;
          // 第二项：-∂ω₂ᶻ/∂x ≈ -(ω₂ᶻ|_{ex} - ω₂ᶻ|_{ex-1}) / hx
          // ω₂ᶻ 在 xy 平面上（法向 z），即 BACK 和 FRONT 面
          curl_y -= (arrOmega2Prev[ez][ey][ex][slot_omega2_z] - arrOmega2Prev[ez][ey][ex-1][slot_omega2_z]) / hx;
          ...
          // 使用四个位置的双线性插值（在 x 和 z 两个方向上）
          PetscScalar omega1_x_z = 0.25 * (arrOmega1[ez][ey][ex][slot_omega1_x] +        // BACK_DOWN (当前单元, 本地)
                                            arrOmega1[ez+1][ey][ex][slot_omega1_x] +      // FRONT_DOWN (前邻居, 前方)
                                            arrOmega1[ez][ey][ex-1][slot_omega1_x] +      // 左邻居的 BACK_DOWN (左边)
                                            arrOmega1[ez+1][ey][ex-1][slot_omega1_x]);    // 左前邻居的 BACK_DOWN (左前)
          
          // ω₁ʸ：在 y 方向棱上，需要插值到 z 方向棱的位置
          // z 方向棱（DOWN_LEFT）位于 x=prev, y=prev，z 坐标是单元中心 ez
          // 使用四个位置的双线性插值（在 y 和 z 两个方向上）
          PetscScalar omega1_y_z = 0.25 * (arrOmega1[ez][ey][ex][slot_omega1_y] +        // BACK_LEFT (当前单元, 本地)
                                            arrOmega1[ez+1][ey][ex][slot_omega1_y] +      // FRONT_LEFT (前邻居, 前方)
                                            arrOmega1[ez][ey-1][ex][slot_omega1_y] +      // 下邻居的 BACK_LEFT (下邻居)
                                            arrOmega1[ez+1][ey-1][ex][slot_omega1_y]);     // 前下邻居的 BACK_LEFT (前下邻居)

          // u₁ʸ^{k-1/2}：需要插值到 z 方向棱的位置，使用四个 y 方向棱（在 y 和 z 两个方向上）
          PetscScalar u1_y_prev_z = 0.25 * (arrU1Prev[ez][ey][ex][slot_u1_y] +        // BACK_LEFT (当前单元, 本地)
                                            arrU1Prev[ez+1][ey][ex][slot_u1_y] +      // FRONT_LEFT (前邻居, 前方)
                                            arrU1Prev[ez][ey-1][ex][slot_u1_y] +      // 下邻居的 BACK_LEFT (下邻居)
                                            arrU1Prev[ez+1][ey-1][ex][slot_u1_y]);     // 前下邻居的 BACK_LEFT (前下邻居)

          // u₁ˣ^{k-1/2}：需要插值到 z 方向棱的位置，使用四个 x 方向棱（在 x 和 z 两个方向上）
          PetscScalar u1_x_prev_z = 0.25 * (arrU1Prev[ez][ey][ex][slot_u1_x] +        // BACK_DOWN (当前单元, 本地)
                                            arrU1Prev[ez+1][ey][ex][slot_u1_x] +      // FRONT_DOWN (前邻居, 前方)
                                            arrU1Prev[ez][ey][ex-1][slot_u1_x] +      // 左邻居的 BACK_DOWN (左边)
                                            arrU1Prev[ez+1][ey][ex-1][slot_u1_x]);    // 左前邻居的 BACK_DOWN (左前)

          // 对流项：0.5 * (ω₁ˣ u₁ʸ - ω₁ʸ u₁ˣ)（0.5 来自时间平均）
          ...
          curl_z += (arrOmega2Prev[ez][ey][ex][slot_omega2_y] - arrOmega2Prev[ez][ey][ex-1][slot_omega2_y]) / hx;
          // 第二项：-∂ω₂ˣ/∂y ≈ -(ω₂ˣ|_{ey} - ω₂ˣ|_{ey-1}) / hy
          // ω₂ˣ 在 yz 平面上（法向 x），即 LEFT 和 RIGHT 面
          curl_z -= (arrOmega2Prev[ez][ey][ex][slot_omega2_x] - arrOmega2Prev[ez][ey-1][ex][slot_omega2_x]) / hy;
    ...
    PetscCall(DMStagVecRestoreArrayRead(dmSol_1, localOmega2Prev, &arrOmega2Prev));
...
// 组装2形式外力项
// 在每一个棱上取对应点和方向的外力值
PetscErrorCode DUAL_MAC::assemble_force1_vector(DM dmSol_1, Vec rhs, ExternalForce externalForce, PetscReal time)
{
    PetscFunctionBeginUser;
    PetscInt startx, starty, startz, nx, ny, nz;
    PetscCall(DMStagGetCorners(dmSol_1, &startx, &starty, &startz,
    ...
    PetscCall(DMStagGetProductCoordinateLocationSlot(dmSol_1, ELEMENT, &icx_center));  // x坐标在单元中心
    PetscCall(DMStagGetProductCoordinateLocationSlot(dmSol_1, LEFT, &icx_prev));        // x坐标在左面
    PetscCall(DMStagGetProductCoordinateLocationSlot(dmSol_1, ELEMENT, &icy_center));  // y坐标在单元中心
    PetscCall(DMStagGetProductCoordinateLocationSlot(dmSol_1, LEFT, &icy_prev));       // y坐标在下面
    PetscCall(DMStagGetProductCoordinateLocationSlot(dmSol_1, ELEMENT, &icz_center));  // z坐标在单元中心
    PetscCall(DMStagGetProductCoordinateLocationSlot(dmSol_1, LEFT, &icz_prev));       // z坐标在后面
    ...
    for (PetscInt ez = startz; ez < startz + nz; ++ez) {
      for (PetscInt ey = starty; ey < starty + ny; ++ey) {
        for (PetscInt ex = startx; ex < startx + nx; ++ex) {
          ...
          PetscCall(DMStagVecSetValuesStencil(dmSol_1, rhs, 1, &row, &force_val, ADD_VALUES));
...
// 组装u1时间矩阵
PetscErrorCode DUAL_MAC::assemble_u1_dt_matrix(DM dmSol_1, Mat A, PetscReal dt)
{
    PetscFunctionBeginUser;
    ...
    for (PetscInt ez = startz; ez < startz + nz; ++ez) {
      for (PetscInt ey = starty; ey < starty + ny; ++ey) {
        for (PetscInt ex = startx; ex < startx + nx; ++ex) {
          ...
          row.loc = BACK_DOWN;
          PetscCall(DMStagMatSetValuesStencil(dmSol_1, A,
                    1, &row,   // 1 行
                    1, &row,   // 1 列（行列相同 → 对角元素）
                    &coeff, ADD_VALUES));
...
PetscErrorCode DUAL_MAC::assemble_u1_conv_matrix(DM dmSol_1, Mat A, DM dmSol_2, Vec omega1_known)
{
    PetscFunctionBeginUser;
    PetscInt startx, starty, startz, nx, ny, nz;
    ...
    for (PetscInt ez = startz; ez < startz + nz; ++ez) {
      for (PetscInt ey = starty; ey < starty + ny; ++ey) {
        for (PetscInt ex = startx; ex < startx + nx; ++ex) {
          ...
          // 2. 当前单元的 BACK_RIGHT = 右邻居的 BACK_LEFT (x=next, y=prev, z=prev)
          // 3. 下方单元的 BACK_LEFT (x=prev, y=prev-1, z=prev)
          // 4. 下方单元的 BACK_RIGHT = 右下邻居的 BACK_LEFT (x=next, y=prev-1, z=prev)
          omega1_y = 0.25 * (arrOmega1[ez][ey][ex][slot_omega1_y] +        // BACK_LEFT (当前单元)
                             arrOmega1[ez][ey][ex+1][slot_omega1_y] +      // BACK_RIGHT (右邻居的BACK_LEFT)
                             arrOmega1[ez][ey-1][ex][slot_omega1_y] +      // 下邻居的 BACK_LEFT
                             arrOmega1[ez][ey-1][ex+1][slot_omega1_y]);    // 右下邻居的 BACK_LEFT
          
          // ω₁ᶻ：在 z 方向棱上，需要插值到 x 方向棱的位置
          // x 方向棱（BACK_DOWN）位于 y=prev, z=prev（后面）
          // 使用四个位置的双线性插值（在 x 和 z 两个方向上）
          // 注意：ez 是当前单元的 z 索引，ez-1 是后邻居（更靠后），ez+1 是前邻居（更靠前）
          // BACK_DOWN 位于 z=prev（后面），所以应该使用当前层（ez）和后一层（ez-1）
          ...
          // 4. 后邻居的 DOWN_RIGHT (x=next, y=prev, z=后一层) - 后右邻居的 DOWN_LEFT
          omega1_z = 0.25 * (arrOmega1[ez][ey][ex][slot_omega1_z] +        // DOWN_LEFT (当前单元)
                             arrOmega1[ez][ey][ex+1][slot_omega1_z] +      // DOWN_RIGHT (右邻居的DOWN_LEFT)
                             arrOmega1[ez-1][ey][ex][slot_omega1_z] +      // 后邻居的 DOWN_LEFT (更靠后)
                             arrOmega1[ez-1][ey][ex+1][slot_omega1_z]);    // 后右邻居的 DOWN_LEFT (更靠后)
          ...
          // 第一项：+ω₁ʸ u₁ᶻ（插值）
          // u₁ᶻ 需要插值到 x 方向棱的位置，使用四个 z 方向棱（在 x 和 z 两个方向上）
          // 1. DOWN_LEFT (当前单元)
          ...
          // 第二项：-ω₁ᶻ u₁ʸ（插值）
          // u₁ʸ 需要插值到 x 方向棱的位置，使用四个 y 方向棱（在 x 和 y 两个方向上）
          // 1. BACK_LEFT (当前单元)
          col[nCol].i = ex;  col[nCol].j = ey;  col[nCol].k = ez;
          ...
          // 使用四个位置的双线性插值（在 y 和 z 两个方向上）
          // 注意：BACK_LEFT 位于 z=prev（后面），所以应该使用当前层（ez）和后一层（ez-1）
          // 1. DOWN_LEFT (x=prev, y=prev, z=当前层) - 当前单元
          // 2. UP_LEFT (x=prev, y=next, z=当前层) - 上邻居的 DOWN_LEFT
          // 3. 后邻居的 DOWN_LEFT (x=prev, y=prev, z=后一层) - 后邻居
          // 4. 后上邻居的 DOWN_LEFT (x=prev, y=next, z=后一层) - 后上邻居
          omega1_z_y = 0.25 * (arrOmega1[ez][ey][ex][slot_omega1_z] +        // DOWN_LEFT (当前单元)
                               arrOmega1[ez][ey+1][ex][slot_omega1_z] +      // UP_LEFT (上邻居的DOWN_LEFT)
                               arrOmega1[ez-1][ey][ex][slot_omega1_z] +      // 后邻居的 DOWN_LEFT (更靠后)
                               arrOmega1[ez-1][ey+1][ex][slot_omega1_z]);    // 后上邻居的 DOWN_LEFT (更靠后)
          ...
          omega1_x = 0.25 * (arrOmega1[ez][ey][ex][slot_omega1_x] +          // BACK_DOWN (当前单元)
                             arrOmega1[ez][ey][ex-1][slot_omega1_x] +        // 左邻居的 BACK_DOWN (更靠左)
                             arrOmega1[ez][ey+1][ex][slot_omega1_x] +        // BACK_UP (上邻居的BACK_DOWN)
                             arrOmega1[ez][ey+1][ex-1][slot_omega1_x]);      // 左上邻居的 BACK_DOWN
          ...
          // 第一项：+ω₁ᶻ u₁ˣ（插值）
          // u₁ˣ 需要插值到 y 方向棱的位置，使用四个 x 方向棱（在 x 和 y 两个方向上）
          // y 方向棱（BACK_LEFT）位于 x=prev, z=prev，所以应该在 x 和 y 方向上插值
          ...
          // 第二项：-ω₁ˣ u₁ᶻ（插值）
          // u₁ᶻ 需要插值到 y 方向棱的位置，使用四个 z 方向棱（在 y 和 z 两个方向上）
          // y 方向棱（BACK_LEFT）位于 x=prev, z=prev，所以应该在 y 和 z 方向上插值
          ...
          omega1_x_z = 0.25 * (arrOmega1[ez][ey][ex][slot_omega1_x] +        // BACK_DOWN (当前单元, 本地)
                               arrOmega1[ez+1][ey][ex][slot_omega1_x] +      // FRONT_DOWN (前邻居, 前方)
                               arrOmega1[ez][ey][ex-1][slot_omega1_x] +      // 左邻居的 BACK_DOWN (左边)
                               arrOmega1[ez+1][ey][ex-1][slot_omega1_x]);    // 左前邻居的 BACK_DOWN (左前)
          ...
          omega1_y_z = 0.25 * (arrOmega1[ez][ey][ex][slot_omega1_y] +        // BACK_LEFT (当前单元, 本地)
                               arrOmega1[ez+1][ey][ex][slot_omega1_y] +      // FRONT_LEFT (前邻居, 前方)
                               arrOmega1[ez][ey-1][ex][slot_omega1_y] +      // 下邻居的 BACK_LEFT (下邻居)
                               arrOmega1[ez+1][ey-1][ex][slot_omega1_y]);     // 前下邻居的 BACK_LEFT (前下邻居)
          ...
          // 第一项：+ω₁ˣ u₁ʸ（插值）
          // u₁ʸ 需要插值到 z 方向棱的位置，使用四个 y 方向棱（在 y 和 z 两个方向上）
          // z 方向棱（DOWN_LEFT）位于 x=prev, y=prev，所以应该在 y 和 z 方向上插值
          // 1. 本地 y 方向：BACK_LEFT (当前单元, x=prev, y=prev, z=当前层)
          col[nCol].i = ex;  col[nCol].j = ey;  col[nCol].k = ez;
          col[nCol].loc = BACK_LEFT;
          col[nCol].c = 0;
          ...
          // 第二项：-ω₁ʸ u₁ˣ（插值）
          // u₁ˣ 需要插值到 z 方向棱的位置，使用四个 x 方向棱（在 x 和 z 两个方向上）
          // z 方向棱（DOWN_LEFT）位于 x=prev, y=prev，所以应该在 x 和 z 方向上插值
          // 1. 本地 x 方向：BACK_DOWN (当前单元, x=prev, y=prev, z=当前层)
          col[nCol].i = ex;  col[nCol].j = ey;  col[nCol].k = ez;
          col[nCol].loc = BACK_DOWN;
          col[nCol].c = 0;
...
PetscErrorCode DUAL_MAC::assemble_omega2_curl_matrix(PetscReal Re, DM dmSol_1, Mat A)
{
    PetscFunctionBeginUser;
    PetscInt startx, starty, startz, nx, ny, nz;
    PetscCall(DMStagGetCorners(dmSol_1, &startx, &starty, &startz,
    ...
    for (PetscInt ez = startz; ez < startz + nz; ++ez) {
      for (PetscInt ey = starty; ey < starty + ny; ++ey) {
        for (PetscInt ex = startx; ex < startx + nx; ++ex) {
          ...
          col[1].i = ex + 1;  col[1].j = ey;  col[1].k = ez;
          ...
          col[1].loc = BACK_DOWN_LEFT;  // 上邻居的左下后顶点 = 本单元的左下后顶点（y方向）
          col[1].c = 0;
          val[1] = 1.0 / hy;
          ...
          col[1].loc = BACK_DOWN_LEFT;  // 前邻居的左下后顶点 = 本单元的左下后顶点（z方向）
          col[1].c = 0;
          val[1] = 1.0 / hz;
    ...
    for (PetscInt ez = startz; ez < startz + nz; ++ez) {
      for (PetscInt ey = starty; ey < starty + ny; ++ey) {
        for (PetscInt ex = startx; ex < startx + nx; ++ex) {
          ...
          // ===== x 方向面 (LEFT/RIGHT) 的旋度方程 =====
          // (∇×u₁)ₓ = ∂u₁ᶻ/∂y - ∂u₁ʸ/∂z = ω₂ₓ
          // 行：x 方向面（LEFT 面，法向 x，位于 x=prev）
          DMStagStencil row;
          row.i = ex;  row.j = ey;  row.k = ez;
          row.loc = LEFT;  // 左面（x=prev，yz平面）
          row.c = 0;       // ω₂ 在 dof2 的第0个分量

          // 列：4条棱的 u₁ + 当前面的 ω₂ 对角项（-1）
          DMStagStencil col[5];
          PetscScalar val[5];
          PetscInt nCol = 0;

          // 第一项：∂u₁ᶻ/∂y ≈ (u₁ᶻ|_{y=next} - u₁ᶻ|_{y=prev}) / hy
          // u₁ᶻ 在 z 方向棱上（DOWN_LEFT）
          // LEFT 面位于 x=prev，使用向前差分
          // 使用本地的 z 方向棱和上方单元的 z 方向棱
          col[nCol].i = ex;  col[nCol].j = ey;     col[nCol].k = ez;
          col[nCol].loc = DOWN_LEFT;  // 本地的 z 方向棱（y=prev）
          col[nCol].c = 0;             // u₁ᶻ 在 dof1 的第0个分量
          val[nCol] = -1.0 / hy;       // 负号因为 (u₁ᶻ|_{y=next} - u₁ᶻ|_{y=prev})
          nCol++;

          col[nCol].i = ex;  col[nCol].j = ey + 1; col[nCol].k = ez;
          col[nCol].loc = DOWN_LEFT;  // 上方单元的 z 方向棱（y=next）
          col[nCol].c = 0;
          val[nCol] = 1.0 / hy;        // 正号因为 (u₁ᶻ|_{y=next} - u₁ᶻ|_{y=prev})
          nCol++;

          // 第二项：-∂u₁ʸ/∂z ≈ -(u₁ʸ|_{z=next} - u₁ʸ|_{z=prev}) / hz
          // u₁ʸ 在 y 方向棱上（BACK_LEFT）
          // LEFT 面位于 z=prev，使用向前差分
          // 使用本地的 y 方向棱和前方单元的 y 方向棱
          col[nCol].i = ex;  col[nCol].j = ey;  col[nCol].k = ez;
          col[nCol].loc = BACK_LEFT;  // 本地的 y 方向棱（z=prev）
          col[nCol].c = 0;             // u₁ʸ 在 dof1 的第0个分量
          val[nCol] = 1.0 / hz;        // 正号来自 -(u₁ʸ|_{z=next} - u₁ʸ|_{z=prev}) = (u₁ʸ|_{z=prev} - u₁ʸ|_{z=next}) / hz
          nCol++;

          col[nCol].i = ex;  col[nCol].j = ey;  col[nCol].k = ez + 1;
          col[nCol].loc = BACK_LEFT;  // 前方单元的 y 方向棱（z=next）
          col[nCol].c = 0;
          val[nCol] = -1.0 / hz;       // 负号来自 -(u₁ʸ|_{z=next} - u₁ʸ|_{z=prev}) = (u₁ʸ|_{z=prev} - u₁ʸ|_{z=next}) / hz
          nCol++;
          ...
          // ===== y 方向面 (DOWN/UP) 的旋度方程 =====
          // (∇×u₁)ᵧ = ∂u₁ˣ/∂z - ∂u₁ᶻ/∂x = ω₂ᵧ
          // 行：y 方向面（DOWN 面，法向 y，位于 y=prev）
          row.loc = DOWN;  // 下面（y=prev，xz平面）
          nCol = 0;

          // 第一项：∂u₁ˣ/∂z ≈ (u₁ˣ|_{z=next} - u₁ˣ|_{z=prev}) / hz
          // u₁ˣ 在 x 方向棱上（BACK_DOWN）
          // DOWN 面位于 z=prev，使用向前差分
          // 使用本地的 x 方向棱和前方单元的 x 方向棱
          col[nCol].i = ex;  col[nCol].j = ey;  col[nCol].k = ez;
          col[nCol].loc = BACK_DOWN;  // 本地的 x 方向棱（z=prev）
          col[nCol].c = 0;             // u₁ˣ 在 dof1 的第0个分量
          val[nCol] = -1.0 / hz;       // 负号因为 (u₁ˣ|_{z=next} - u₁ˣ|_{z=prev})
          nCol++;

          col[nCol].i = ex;  col[nCol].j = ey;  col[nCol].k = ez + 1;
          col[nCol].loc = BACK_DOWN;  // 前方单元的 x 方向棱（z=next）
          col[nCol].c = 0;
          val[nCol] = 1.0 / hz;        // 正号因为 (u₁ˣ|_{z=next} - u₁ˣ|_{z=prev})
          nCol++;

          // 第二项：-∂u₁ᶻ/∂x ≈ -(u₁ᶻ|_{x=next} - u₁ᶻ|_{x=prev}) / hx
          // u₁ᶻ 在 z 方向棱上（DOWN_LEFT）
          ...
          col[nCol].c = 0;             // u₁ᶻ 在 dof1 的第0个分量
          val[nCol] = 1.0 / hx;        // 正号来自 -(u₁ᶻ|_{x=next} - u₁ᶻ|_{x=prev}) = (u₁ᶻ|_{x=prev} - u₁ᶻ|_{x=next}) / hx
          nCol++;

          col[nCol].i = ex + 1; col[nCol].j = ey;  col[nCol].k = ez;
          col[nCol].loc = DOWN_LEFT;  // 右边单元的 z 方向棱（x=next）
          col[nCol].c = 0;
          val[nCol] = -1.0 / hx;       // 负号来自 -(u₁ᶻ|_{x=next} - u₁ᶻ|_{x=prev}) = (u₁ᶻ|_{x=prev} - u₁ᶻ|_{x=next}) / hx
          nCol++;
          ...
          // ===== z 方向面 (BACK/FRONT) 的旋度方程 =====
          // (∇×u₁)ᵧ = ∂u₁ʸ/∂x - ∂u₁ˣ/∂y = ω₂ᵧ
          // 行：z 方向面（BACK 面，法向 z，位于 z=prev）
          row.loc = BACK;  // 后面（z=prev，xy平面）
          nCol = 0;

          // 第一项：∂u₁ʸ/∂x ≈ (u₁ʸ|_{x=next} - u₁ʸ|_{x=prev}) / hx
          // u₁ʸ 在 y 方向棱上（BACK_LEFT）
          ...
          col[nCol].c = 0;             // u₁ʸ 在 dof1 的第0个分量
          val[nCol] = -1.0 / hx;       // 负号因为 (u₁ʸ|_{x=next} - u₁ʸ|_{x=prev})
          nCol++;

          col[nCol].i = ex + 1; col[nCol].j = ey;  col[nCol].k = ez;
          col[nCol].loc = BACK_LEFT;  // 右边单元的 y 方向棱（x=next）
          col[nCol].c = 0;
          val[nCol] = 1.0 / hx;        // 正号因为 (u₁ʸ|_{x=next} - u₁ʸ|_{x=prev})
          nCol++;

          // 第二项：-∂u₁ˣ/∂y ≈ -(u₁ˣ|_{y=next} - u₁ˣ|_{y=prev}) / hy
          // u₁ˣ 在 x 方向棱上（BACK_DOWN）
          // BACK 面位于 y=prev，使用向前差分
          // 使用本地的 x 方向棱和上方单元的 x 方向棱
          col[nCol].i = ex;  col[nCol].j = ey;     col[nCol].k = ez;
          col[nCol].loc = BACK_DOWN;  // 本地的 x 方向棱（y=prev）
          col[nCol].c = 0;             // u₁ˣ 在 dof1 的第0个分量
          val[nCol] = 1.0 / hy;       // 负号来自 -(u₁ˣ|_{y=next} - u₁ˣ|_{y=prev})
          nCol++;

          col[nCol].i = ex;  col[nCol].j = ey + 1; col[nCol].k = ez;
          col[nCol].loc = BACK_DOWN;  // 上方单元的 x 方向棱（y=next）
          col[nCol].c = 0;
          val[nCol] = -1.0 / hy;        // 正号来自 -(u₁ˣ|_{y=next} - u₁ˣ|_{y=prev})
          nCol++;
