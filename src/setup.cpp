...
DUAL_MAC::DUAL_MAC(PetscReal time, PetscReal nu, PetscInt NX, PetscInt NY,
                   PetscInt NZ, PetscInt Nt, PetscReal xmin, PetscReal xmax,
                   PetscReal ymin, PetscReal ymax, PetscReal zmin,
                   PetscReal zmax, PetscBool pinPressure)
    : DUAL_MAC() {
  this->time = time;
  this->nu = nu;
  this->Nx = NX;
...
PetscErrorCode DUAL_MAC::set_dm_periodic() {
  ...
  // 创建1形式的dm
  const PetscInt dof0 = 1, dof1 = 1, dof2 = 1,
                 dof3 = 0; /* 1 dof on each face and element center */
  const PetscInt stencilWidth = 1;
  PetscCall(DMStagCreate3d(
      PETSC_COMM_WORLD, DM_BOUNDARY_PERIODIC, DM_BOUNDARY_PERIODIC,
      DM_BOUNDARY_PERIODIC, this->Nx, this->Ny, this->Nz, PETSC_DECIDE,
      PETSC_DECIDE, PETSC_DECIDE, dof0, dof1, dof2, dof3, DMSTAG_STENCIL_BOX,
      stencilWidth, NULL, NULL, NULL, &dmSol_1));
  ...
  PetscCall(DMStagCreateCompatibleDMStag(dmSol_1, dof0_2, dof1_2, dof2_2,
                                         dof3_2, &dmSol_2));
...
PetscErrorCode DUAL_MAC::set_stabilization_parameters(PetscReal alpha,
                                                      PetscReal gamma) {
  PetscFunctionBeginUser;
  this->stab_alpha = alpha;
  this->stab_gamma = gamma;
  PetscFunctionReturn(PETSC_SUCCESS);
}

// 打印DM信息（用于调试）
PetscErrorCode DUAL_MAC::view_dm(PetscBool view_dm1, PetscBool view_dm2,
                                 PetscViewer viewer) {
  PetscFunctionBeginUser;
  ...
  if (view_dm1 && dmSol_1) {
    PetscCall(PetscPrintf(PETSC_COMM_WORLD,
                          "\n========================================\n"));
    PetscCall(
        PetscPrintf(PETSC_COMM_WORLD, "DM Sol 1 (1-form system) 信息:\n"));
    PetscCall(PetscPrintf(PETSC_COMM_WORLD,
                          "========================================\n"));
  ...
  if (view_dm2 && dmSol_2) {
    PetscCall(PetscPrintf(PETSC_COMM_WORLD,
                          "========================================\n"));
    PetscCall(
        PetscPrintf(PETSC_COMM_WORLD, "DM Sol 2 (2-form system) 信息:\n"));
    PetscCall(PetscPrintf(PETSC_COMM_WORLD,
                          "========================================\n"));
