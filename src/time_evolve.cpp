...
static PetscErrorCode solve_linear_system_with_switch(Mat A, Vec rhs, Vec sol, DM dm, const char *optionsPrefix)
{
    ...
    if (optionsPrefix && optionsPrefix[0] != '\0') {
        PetscCall(PetscOptionsGetBool(NULL, optionsPrefix, "-use_graddiv", &useGradDivLocal, &hasUseLocal));
        PetscCall(PetscOptionsGetReal(NULL, optionsPrefix, "-graddiv_gamma", &gammaLocal, &hasGammaLocal));
    }
    PetscCall(PetscOptionsGetBool(NULL, NULL, "-use_graddiv", &useGradDivGlobal, &hasUseGlobal));
    PetscCall(PetscOptionsGetReal(NULL, NULL, "-graddiv_gamma", &gammaGlobal, &hasGammaGlobal));

    const PetscBool useGradDiv = hasUseLocal ? useGradDivLocal : (hasUseGlobal ? useGradDivGlobal : PETSC_FALSE);
    const PetscReal gamma = hasGammaLocal ? gammaLocal : (hasGammaGlobal ? gammaGlobal : 0.0);
...
static PetscErrorCode debug_check_vec_finite(Vec v, const char *tag)
{
    ...
    for (PetscInt i = 0; i < nLocal; ++i) {
        ...
        const bool finiteVal = std::isfinite((double)PetscRealPart(val)) && std::isfinite((double)PetscImaginaryPart(val));
#else
    ...
    {
        PetscMPIInt ierr = MPI_Allreduce(&localFirst, &globalFirst, 1, MPIU_INT, MPI_MIN, comm);
        if (ierr) SETERRQ(comm, PETSC_ERR_LIB, "MPI_Allreduce failed for first invalid index");
    }
    ...
    if (localFirst == globalFirst) owner = rank;
    {
        PetscMPIInt ierr = MPI_Allreduce(MPI_IN_PLACE, &owner, 1, MPI_INT, MPI_MIN, comm);
        if (ierr) SETERRQ(comm, PETSC_ERR_LIB, "MPI_Allreduce failed for owner rank");
    }
    if (rank != owner) localVal = 0.0;
    {
        PetscMPIInt ierr = MPI_Bcast(&localVal, 1, MPIU_SCALAR, owner, comm);
        if (ierr) SETERRQ(comm, PETSC_ERR_LIB, "MPI_Bcast failed for invalid value");
    }

#if defined(PETSC_USE_COMPLEX)
    PetscCall(PetscPrintf(PETSC_COMM_WORLD,
                          "[DEBUG][finite] %s: first invalid at global idx=%" PetscInt_FMT ", value=(%.16e, %.16e)\n",
                          tag ? tag : "vec", globalFirst, (double)PetscRealPart(localVal), (double)PetscImaginaryPart(localVal)));
#else
    PetscCall(PetscPrintf(PETSC_COMM_WORLD,
                          "[DEBUG][finite] %s: first invalid at global idx=%" PetscInt_FMT ", value=%.16e\n",
                          tag ? tag : "vec", globalFirst, (double)PetscRealPart(localVal)));
#endif
    PetscFunctionReturn(PETSC_SUCCESS);
}

static PetscErrorCode dump_matrix_ascii_matlab_debug(Mat A, const char *tag, PetscInt step_id)
{
    PetscFunctionBeginUser;
    ...
    oss << "mat/" << tag << "_step_" << std::setw(4) << std::setfill('0') << step_id << ".m";
...
static PetscErrorCode dump_vector_ascii_matlab_debug(Vec v, const char *tag, PetscInt step_id)
{
    PetscFunctionBeginUser;
    ...
    std::ostringstream oss;
    oss << "vec/" << tag << "_step_" << std::setw(4) << std::setfill('0') << step_id << ".m";
    const std::string filename = oss.str();
...
static PetscErrorCode debug_report_curl_consistency(DM dmSol_1, Vec sol1, DM dmSol_2, Vec sol2,
                                                    PetscReal hx, PetscReal hy, PetscReal hz, PetscReal cellVolume)
{
    PetscFunctionBeginUser;
    ...
    PetscCall(DMStagGetCorners(dmSol_1, &sx1, &sy1, &sz1, &nx1, &ny1, &nz1, NULL, NULL, NULL));
        ...
        for (PetscInt ey = sy1; ey < sy1 + ny1; ++ey) {
            for (PetscInt ex = sx1; ex < sx1 + nx1; ++ex) {
                const PetscReal curl_x =
                    (PetscRealPart(arrSol1[ez][ey + 1][ex][slot_u1_z]) - PetscRealPart(arrSol1[ez][ey][ex][slot_u1_z])) / hy
                    - (PetscRealPart(arrSol1[ez + 1][ey][ex][slot_u1_y]) - PetscRealPart(arrSol1[ez][ey][ex][slot_u1_y])) / hz;
                const PetscReal curl_y =
                    (PetscRealPart(arrSol1[ez + 1][ey][ex][slot_u1_x]) - PetscRealPart(arrSol1[ez][ey][ex][slot_u1_x])) / hz
                    - (PetscRealPart(arrSol1[ez][ey][ex + 1][slot_u1_z]) - PetscRealPart(arrSol1[ez][ey][ex][slot_u1_z])) / hx;
                const PetscReal curl_z =
                    (PetscRealPart(arrSol1[ez][ey][ex + 1][slot_u1_y]) - PetscRealPart(arrSol1[ez][ey][ex][slot_u1_y])) / hx
                    - (PetscRealPart(arrSol1[ez][ey + 1][ex][slot_u1_x]) - PetscRealPart(arrSol1[ez][ey][ex][slot_u1_x])) / hy;

                const PetscReal d0 = PetscRealPart(arrSol1[ez][ey][ex][slot_omega2_x]) - curl_x;
                const PetscReal d1 = PetscRealPart(arrSol1[ez][ey][ex][slot_omega2_y]) - curl_y;
                const PetscReal d2 = PetscRealPart(arrSol1[ez][ey][ex][slot_omega2_z]) - curl_z;
    ...
    PetscCall(DMStagGetCorners(dmSol_2, &sx2, &sy2, &sz2, &nx2, &ny2, &nz2, NULL, NULL, NULL));
    ...
    for (PetscInt ez = sz2; ez < sz2 + nz2; ++ez) {
        for (PetscInt ey = sy2; ey < sy2 + ny2; ++ey) {
            for (PetscInt ex = sx2; ex < sx2 + nx2; ++ex) {
                ...
                const PetscReal curl_x =
                    (PetscRealPart(arrSol2[ez][ey][ex][slot_u2_z]) - PetscRealPart(arrSol2[ez][ey - 1][ex][slot_u2_z])) / hy
                    - (PetscRealPart(arrSol2[ez][ey][ex][slot_u2_y]) - PetscRealPart(arrSol2[ez - 1][ey][ex][slot_u2_y])) / hz;
                // curl_y = d(u2_x)/dz - d(u2_z)/dx，
                // 分别来自 LEFT 面在 z 向差分与 BACK 面在 x 向差分。
                const PetscReal curl_y =
                    (PetscRealPart(arrSol2[ez][ey][ex][slot_u2_x]) - PetscRealPart(arrSol2[ez - 1][ey][ex][slot_u2_x])) / hz
                    - (PetscRealPart(arrSol2[ez][ey][ex][slot_u2_z]) - PetscRealPart(arrSol2[ez][ey][ex - 1][slot_u2_z])) / hx;
                // curl_z = d(u2_y)/dx - d(u2_x)/dy，
                // 分别来自 DOWN 面在 x 向差分与 LEFT 面在 y 向差分。
                const PetscReal curl_z =
                    (PetscRealPart(arrSol2[ez][ey][ex][slot_u2_y]) - PetscRealPart(arrSol2[ez][ey][ex - 1][slot_u2_y])) / hx
                    - (PetscRealPart(arrSol2[ez][ey][ex][slot_u2_x]) - PetscRealPart(arrSol2[ez][ey - 1][ex][slot_u2_x])) / hy;

                const PetscReal d0 = PetscRealPart(arrSol2[ez][ey][ex][slot_omega1_x]) - curl_x;
                const PetscReal d1 = PetscRealPart(arrSol2[ez][ey][ex][slot_omega1_y]) - curl_y;
                const PetscReal d2 = PetscRealPart(arrSol2[ez][ey][ex][slot_omega1_z]) - curl_z;
    ...
    PetscCallMPI(MPI_Allreduce(&localSqOmega2Curl, &globalSqOmega2Curl, 1, MPIU_REAL, MPI_SUM, PETSC_COMM_WORLD));
    PetscCallMPI(MPI_Allreduce(&localSqOmega1Curl, &globalSqOmega1Curl, 1, MPIU_REAL, MPI_SUM, PETSC_COMM_WORLD));
    ...
    PetscCall(PetscPrintf(PETSC_COMM_WORLD,
                          "[DEBUG][consistency] ||omega2-curl(u1)||_L2 = %.12e, ||omega1-curl(u2)||_L2 = %.12e\n",
...
PetscErrorCode DUAL_MAC::setup_initial_solution(RefSol refSol, Vec u1_0, Vec u2_0, Vec omega1_0, Vec omega2_0)
{
    PetscFunctionBeginUser;
    ...
    PetscCall(DMStagGetCorners(dmSol_1, &startx, &starty, &startz,
    ...
    PetscCall(DMStagGetProductCoordinateArraysRead(dmSol_1, &cArrX, &cArrY, &cArrZ));

    // 获取 product 坐标的 slot 索引
    PetscInt icx_center, icx_prev, icy_center, icy_prev, icz_center, icz_prev;
    PetscCall(DMStagGetProductCoordinateLocationSlot(dmSol_1, ELEMENT, &icx_center));
    PetscCall(DMStagGetProductCoordinateLocationSlot(dmSol_1, LEFT, &icx_prev));
    PetscCall(DMStagGetProductCoordinateLocationSlot(dmSol_1, ELEMENT, &icy_center));
    PetscCall(DMStagGetProductCoordinateLocationSlot(dmSol_1, LEFT, &icy_prev));
    PetscCall(DMStagGetProductCoordinateLocationSlot(dmSol_1, ELEMENT, &icz_center));
    ...
    // 根据论文公式：
    // u_1^{h,1/2}/dt + ∇_h P_0^{h,0} = R_1 f^0 - ω_1^{h,0} × ω_1^{h,0} - (1/Re) ∇_h × ω_2^{h,0} + u_1^{h,0}/dt
    // ∇_h · u_1^{h,1/2} = 0
    ...
    PetscCall(DMStagRestoreProductCoordinateArraysRead(dmSol_1, &cArrX, &cArrY, &cArrZ));
...
// 求解系统：
//   u₁^{h,1/2}/dt + ∇P₀^{h,0} = R₁f^0 - ω₁^{h,0}×u₁^{h,0} - (1/Re)∇×ω₂^{h,0} + u₁^{h,0}/dt
//   ∇·u₁^{h,1/2} = 0
// 然后计算：ω₂^{h,1/2} = ∇×u₁^{h,1/2}
PetscErrorCode DUAL_MAC::compute_half_solution(Vec u1_0, Vec omega1_0, Vec omega2_0, 
                                                 ...
                                                 ExternalForce externalForce)
{
    ...
    // 使用 half 专用 DM（仅包含 u1 + p0），避免把 omega2 的自由度放进 half 线性系统
    DM dmHalf = NULL;
    ...
    // ===== 2. 组装矩阵 =====
    // 2.1 时间导数矩阵：1/dt * I（对 u₁）
    DUAL_MAC_DEBUG_LOG("[DEBUG] [1/2步] 组装时间导数矩阵开始\n");
    PetscCall(assemble_u1_dt_matrix(dmHalf, A, this->dt));
    ...
    PetscCall(VecScale(rhs, 1.0 / this->dt));

    // 3.2 外力项：R₁ f^0
    PetscCall(assemble_force1_vector(dmHalf, rhs, externalForce, 0.0));

    // 3.3 对流项：-ω₁^{h,0} × u₁^{h,0}
    PetscCall(assemble_omega1_u1_conv_rhs(dmHalf, rhs, dmSol_2, omega1_0, dmSol_1, u1_0));
    ...
    if (this->pinPressure) {
        ...
        const PetscBool owns_anchor =
            ...
            (0 >= startz_pin && 0 < startz_pin + nz_pin) ? PETSC_TRUE : PETSC_FALSE;
        ...
        prow.c = 0;
        PetscCall(DMStagVecSetValuesStencil(dmHalf, rhs, 1, &prow, &pval, INSERT_VALUES));
        }
        PetscCall(VecAssemblyBegin(rhs));
        PetscCall(VecAssemblyEnd(rhs));
    }
...
PetscErrorCode DUAL_MAC::assemble_omega1_u1_conv_rhs(DM dmRhs, Vec rhs, DM dmSol_2, Vec omega1_0, DM dmU1, Vec u1_0)
{
    PetscFunctionBeginUser;
    PetscInt startx, starty, startz, nx, ny, nz;
    PetscCall(DMStagGetCorners(dmRhs, &startx, &starty, &startz,
                               &nx, &ny, &nz, NULL, NULL, NULL));
    PetscReal hx, hy, hz;
    hx = (this->xmax - this->xmin) / static_cast<PetscReal>(this->Nx);
    hy = (this->ymax - this->ymin) / static_cast<PetscReal>(this->Ny);
    ...
    for (PetscInt ez = startz; ez < startz + nz; ++ez) {
      for (PetscInt ey = starty; ey < starty + ny; ++ey) {
        for (PetscInt ex = startx; ex < startx + nx; ++ex) {
          ...
          // 插值 ω₁ 到 x 方向棱的位置（与 assemble_rhs1_vector/assemble_u1_conv_matrix 中的逻辑相同）
          PetscScalar omega1_y = 0.25 * (arrOmega1[ez][ey][ex][slot_omega1_y] +
                                         arrOmega1[ez][ey][ex+1][slot_omega1_y] +
                                         arrOmega1[ez][ey-1][ex][slot_omega1_y] +
                                         arrOmega1[ez][ey-1][ex+1][slot_omega1_y]);

          PetscScalar omega1_z = 0.25 * (arrOmega1[ez][ey][ex][slot_omega1_z] +
          ...
          PetscCall(DMStagVecSetValuesStencil(dmRhs, rhs, 1, &row, &rhs_val, ADD_VALUES));

          // ===== y 方向棱 (BACK_LEFT) 的 -ω₁ × u₁ 项 =====
          row.loc = BACK_LEFT;
          // 插值逻辑（与 assemble_rhs1_vector/assemble_u1_conv_matrix 中的逻辑相同）
          PetscScalar omega1_z_y = 0.25 * (arrOmega1[ez][ey][ex][slot_omega1_z] +
                                            arrOmega1[ez][ey+1][ex][slot_omega1_z] +
                                            arrOmega1[ez-1][ey][ex][slot_omega1_z] +
                                            arrOmega1[ez-1][ey+1][ex][slot_omega1_z]);

          PetscScalar omega1_x_y = 0.25 * (arrOmega1[ez][ey][ex][slot_omega1_x] +
          ...
          row.loc = DOWN_LEFT;
          // 插值逻辑（与 assemble_rhs1_vector/assemble_u1_conv_matrix 中的逻辑相同）
          PetscScalar omega1_x_z = 0.25 * (arrOmega1[ez][ey][ex][slot_omega1_x] +
                                           arrOmega1[ez+1][ey][ex][slot_omega1_x] +
                                           arrOmega1[ez][ey][ex-1][slot_omega1_x] +
                                           arrOmega1[ez+1][ey][ex-1][slot_omega1_x]);

          PetscScalar omega1_y_z = 0.25 * (arrOmega1[ez][ey][ex][slot_omega1_y] +
...
PetscErrorCode DUAL_MAC::assemble_omega2_curl_rhs(DM dmRhs, Vec rhs, DM dmOmega2, Vec omega2_0, PetscReal Re)
{
    PetscFunctionBeginUser;
    PetscInt startx, starty, startz, nx, ny, nz;
    PetscCall(DMStagGetCorners(dmRhs, &startx, &starty, &startz,
    ...
    for (PetscInt ez = startz; ez < startz + nz; ++ez) {
      for (PetscInt ey = starty; ey < starty + ny; ++ey) {
        for (PetscInt ex = startx; ex < startx + nx; ++ex) {
          ...
          curl_x += (arrOmega2[ez][ey][ex][slot_omega2_z] - arrOmega2[ez][ey-1][ex][slot_omega2_z]) / hy;
          // -∂ω₂ʸ/∂z ≈ -(ω₂ʸ|_{ez} - ω₂ʸ|_{ez-1}) / hz
          curl_x -= (arrOmega2[ez][ey][ex][slot_omega2_y] - arrOmega2[ez-1][ey][ex][slot_omega2_y]) / hz;
          ...
          curl_y += (arrOmega2[ez][ey][ex][slot_omega2_x] - arrOmega2[ez-1][ey][ex][slot_omega2_x]) / hz;
          // -∂ω₂ᶻ/∂x ≈ -(ω₂ᶻ|_{ex} - ω₂ᶻ|_{ex-1}) / hx
          curl_y -= (arrOmega2[ez][ey][ex][slot_omega2_z] - arrOmega2[ez][ey][ex-1][slot_omega2_z]) / hx;
          ...
          curl_z += (arrOmega2[ez][ey][ex][slot_omega2_y] - arrOmega2[ez][ey][ex-1][slot_omega2_y]) / hx;
          // -∂ω₂ˣ/∂y ≈ -(ω₂ˣ|_{ey} - ω₂ˣ|_{ey-1}) / hy
          curl_z -= (arrOmega2[ez][ey][ex][slot_omega2_x] - arrOmega2[ez][ey-1][ex][slot_omega2_x]) / hy;
...
PetscErrorCode DUAL_MAC::extract_u1_from_solution(DM dmSolSrc, Vec sol, DM dmSolDst, Vec u1_half)
{
    PetscFunctionBeginUser;
    ...
    PetscInt startx, starty, startz, nx, ny, nz;
    PetscCall(DMStagGetCorners(dmSolDst, &startx, &starty, &startz,
                               &nx, &ny, &nz, NULL, NULL, NULL));
    ...
    for (PetscInt ez = startz; ez < startz + nz; ++ez) {
      for (PetscInt ey = starty; ey < starty + ny; ++ey) {
        for (PetscInt ex = startx; ex < startx + nx; ++ex) {
          arrU1Half[ez][ey][ex][slot_u1_dst_x] = arrSol[ez][ey][ex][slot_u1_src_x];
          arrU1Half[ez][ey][ex][slot_u1_dst_y] = arrSol[ez][ey][ex][slot_u1_src_y];
          arrU1Half[ez][ey][ex][slot_u1_dst_z] = arrSol[ez][ey][ex][slot_u1_src_z];
        }
      }
    }
...
PetscErrorCode DUAL_MAC::compute_curl_u1_to_omega2(DM dmSol_1, Vec u1_half, Vec omega2_half)
{
    PetscFunctionBeginUser;
    PetscInt startx, starty, startz, nx, ny, nz;
    PetscCall(DMStagGetCorners(dmSol_1, &startx, &starty, &startz,
    ...
    PetscCall(DMGlobalToLocal(dmSol_1, omega2_half, INSERT_VALUES, localOmega2Half));
    ...
    for (PetscInt ez = startz; ez < startz + nz; ++ez) {
      for (PetscInt ey = starty; ey < starty + ny; ++ey) {
        for (PetscInt ex = startx; ex < startx + nx; ++ex) {
          ...
          curl_x += (arrU1Half[ez][ey+1][ex][slot_u1_z] - arrU1Half[ez][ey][ex][slot_u1_z]) / hy;
          // -∂u₁ʸ/∂z ≈ -(u₁ʸ|_{z=next} - u₁ʸ|_{z=prev}) / hz
          curl_x -= (arrU1Half[ez+1][ey][ex][slot_u1_y] - arrU1Half[ez][ey][ex][slot_u1_y]) / hz;
          ...
          curl_y += (arrU1Half[ez+1][ey][ex][slot_u1_x] - arrU1Half[ez][ey][ex][slot_u1_x]) / hz;
          // -∂u₁ᶻ/∂x ≈ -(u₁ᶻ|_{x=next} - u₁ᶻ|_{x=prev}) / hx
          curl_y -= (arrU1Half[ez][ey][ex+1][slot_u1_z] - arrU1Half[ez][ey][ex][slot_u1_z]) / hx;
          ...
          curl_z += (arrU1Half[ez][ey][ex+1][slot_u1_y] - arrU1Half[ez][ey][ex][slot_u1_y]) / hx;
          // -∂u₁ˣ/∂y ≈ -(u₁ˣ|_{y=next} - u₁ˣ|_{y=prev}) / hy
          curl_z -= (arrU1Half[ez][ey+1][ex][slot_u1_x] - arrU1Half[ez][ey][ex][slot_u1_x]) / hy;
    ...
    // 写回全局向量
    PetscCall(DMLocalToGlobal(dmSol_1, localOmega2Half, INSERT_VALUES, omega2_half));
...
// 1-形式系统时间推进：给定半步旧解 sol1_old 和整数步新解 sol2_new，求半步新解 sol1_new
// sol1_old : (u1^{k-1/2}, ω2^{k-1/2}, P0^{k-1/2}) 对应的 DM dmSol_1 全局向量
// sol1_new : (u1^{k+1/2}, ω2^{k+1/2}, P0^{k})    求解得到，存回同一 DM 的全局向量
// sol2_old : 整数步旧解 (u2^{k-1}, ω1^{k-1}, P3^{k-3/2}) —— 这里只作为接口占位，不直接使用
// sol2_new : 整数步新解 (u2^{k},   ω1^{k},   P3^{k-1/2})，其中 ω1^{k} 用作对流场
PetscErrorCode DUAL_MAC::time_evolve1(DM dmSol_1,DM dmSol_2,
                                      ...
                                      PetscReal time)
{
    ...
    PetscCall(assemble_1form_system_matrix(dmSol_1, dmSol_2,
                                           A, rhs,
                                           sol1_old,
                                           sol2_new,
                                           sol1_old,
    ...
    PetscCall(solve_linear_system_with_switch(A, rhs, sol1_new, dmSol_1, "one_"));
...
// 2-形式系统时间推进：给定整数步旧解 sol2_old 和半步旧解 sol1_old，求整数步新解 sol2_new
// sol1_old : 半步旧解 (u1^{k-1/2}, ω2^{k-1/2}, P0^{k-1/2})，其中 ω2^{k-1/2} 作为对流场
// sol1_new : 半步新解 (u1^{k+1/2}, ω2^{k+1/2}, P0^{k}) —— 这里只作为接口占位，不直接使用
// sol2_old : 整数步旧解 (u2^{k-1}, ω1^{k-1}, P3^{k-3/2})
// sol2_new : 整数步新解 (u2^{k},   ω1^{k},   P3^{k-1/2}) 求解得到
PetscErrorCode DUAL_MAC::time_evolve2(DM dmSol_1,DM dmSol_2,
                                      Vec sol1_old, Vec sol1_new,
                                      Vec sol2_old, Vec sol2_new,
                                      ExternalForce externalForce,
                                      PetscReal time)
{
    ...
    //    根据论文方程(5.1)，外力项是 I_{RT} f^{k-1/2}，即半整数时间步 k-1/2 的外力
    //    假设传入的 time 是当前整数时间步 k，则半整数时间步是 k - 0.5*dt
    PetscReal time_for_force = time - 0.5 * this->dt;  // 半整数时间步 k-1/2
    DUAL_MAC_DEBUG_LOG("[DEBUG] 2-form 子步矩阵组装开始\n");
    PetscCall(assemble_2form_system_matrix(dmSol_1, dmSol_2,
                                           A, rhs,
                                           sol2_old,
                                           sol1_old,
                                           sol2_old,
...
#if DUAL_MAC_DEBUG
    {
        ...
        PetscCall(debug_check_vec_finite(sol2_new, "twoform_sol_initial_guess_before_zero"));
    }
#endif
    ...
    PetscCall(solve_linear_system_with_switch(A, rhs, sol2_new, dmSol_2, "two_"));
...
PetscErrorCode DUAL_MAC::solve(RefSol refSol, ExternalForce externalForce)
{
    ...
    const PetscReal debugCellVolume = (dx > 0.0 && dy > 0.0 && dz > 0.0) ? (dx * dy * dz) : -1.0;
    const PetscReal hx_dbg = (this->xmax - this->xmin) / static_cast<PetscReal>(this->Nx);
    const PetscReal hy_dbg = (this->ymax - this->ymin) / static_cast<PetscReal>(this->Ny);
    const PetscReal hz_dbg = (this->zmax - this->zmin) / static_cast<PetscReal>(this->Nz);
    ...
    // ===== 3. 创建半步解向量 =====
    // u1_half, omega2_half 存储在 dmSol_1 中
    Vec u1_half, omega2_half;
    PetscCall(DMCreateGlobalVector(dmSol_1, &u1_half));
    PetscCall(DMCreateGlobalVector(dmSol_1, &omega2_half));

    // ===== 4. 计算 1/2 时刻的解 =====
    PetscCall(compute_half_solution(u1_0, omega1_0, omega2_0, 
                                     u1_half, omega2_half, 
                                     externalForce));
#if DUAL_MAC_DEBUG
    PetscCall(PetscPrintf(PETSC_COMM_WORLD, "[DEBUG] 输出 1/2 时刻解向量\n"));
    PetscCall(dump_vector_ascii_matlab_debug(u1_half, "half_u1", 0));
    PetscCall(dump_vector_ascii_matlab_debug(omega2_half, "half_omega2", 0));
#endif
    ...
    // ===== 6. 初始化时间推进的初始状态 =====
    // sol1_old: 从 u1_half 和 omega2_half 组装（t=1/2时刻）
    // sol2_old: 从 u2_0 和 omega1_0 组装（t=0时刻）
    PetscCall(assemble_sol1_from_components(dmSol_1, u1_half, omega2_half, sol1_old));
    PetscCall(assemble_sol2_from_components(dmSol_2, u2_0, omega1_0, sol2_old));
#if DUAL_MAC_DEBUG
    Vec sol1_init = NULL;
    PetscCall(DMCreateGlobalVector(dmSol_1, &sol1_init));
    PetscCall(assemble_sol1_from_components(dmSol_1, u1_0, omega2_0, sol1_init));

    PetscCall(PetscPrintf(PETSC_COMM_WORLD, "[DEBUG] 输出初始整体状态向量(sol1_old/sol2_old)\n"));
    PetscCall(dump_vector_ascii_matlab_debug(sol1_old, "initial_sol1_half_state", 0));
    PetscCall(dump_vector_ascii_matlab_debug(sol2_old, "initial_sol2_state", 0));

    PetscCall(PetscPrintf(PETSC_COMM_WORLD, "[DEBUG] 初值误差评估 (t=0)\n"));
    PetscCall(debugEvaluator.compute_error(dmSol_1, sol1_init, refSol,
                                           0.0, 0.0, debugCellVolume));
    PetscCall(debugEvaluator.compute_error(dmSol_2, sol2_old, refSol,
                                           0.0, 0.0, debugCellVolume));

    PetscCall(PetscPrintf(PETSC_COMM_WORLD, "[DEBUG] 1/2时刻误差评估 (t=%g)\n", (double)(0.5 * this->dt)));
    PetscCall(debugEvaluator.compute_error(dmSol_1, sol1_old, refSol,
                                           0.5 * this->dt, 0.0, debugCellVolume));
    ...
    for (PetscInt k = 1; k <= this->Nt; ++k) {
        ...
        DUAL_MAC_DEBUG_LOG("[DEBUG] 时间步 %d 开始 (t=%g)\n", (int)k, (double)current_time);

        // 7.1 推进 2-形式系统：从 k-1 到 k
        //     输入：sol2_old (k-1), sol1_old (k-1/2)
        //     输出：sol2_new (k)
        PetscCall(time_evolve2(dmSol_1, dmSol_2,
                                sol1_old, sol1_new,  // sol1_old 提供 omega2^{k-1/2}
        ...
        PetscCall(time_evolve1(dmSol_1, dmSol_2,
                                sol1_old, sol1_new,  // sol1_old -> sol1_new
                                sol2_old, sol2_new,  // sol2_new 提供 omega1^k
                                externalForce, time_half));
#if DUAL_MAC_DEBUG
        ...
        PetscCall(debug_report_curl_consistency(dmSol_1, sol1_new, dmSol_2, sol2_new,
                                                hx_dbg, hy_dbg, hz_dbg, debugCellVolume));
        PetscCall(PetscPrintf(PETSC_COMM_WORLD,
                 "[DEBUG] 时间步 %d 误差评估：2-form(t=%g), 1-form(t=%g)\n",
                 (int)k, (double)current_time, (double)time_half));
        PetscCall(debugEvaluator.compute_error(dmSol_2, sol2_new, refSol,
                                               current_time, current_time - 0.5 * this->dt, debugCellVolume));
        PetscCall(debugEvaluator.compute_error(dmSol_1, sol1_new, refSol,
                                               time_half, current_time, debugCellVolume));
#endif
    ...
    PetscCall(VecDestroy(&u1_half));
    PetscCall(VecDestroy(&omega2_half));
...
PetscErrorCode DUAL_MAC::assemble_sol1_from_components(DM dmSol_1, Vec u1, Vec omega2, Vec sol1)
{
    PetscFunctionBeginUser;
    ...
    for (PetscInt ez = startz; ez < startz + nz; ++ez) {
      for (PetscInt ey = starty; ey < starty + ny; ++ey) {
        for (PetscInt ex = startx; ex < startx + nx; ++ex) {
          ...
          // 复制 omega2
          arrSol1[ez][ey][ex][slot_omega2_x] = arrOmega2[ez][ey][ex][slot_omega2_x];
          arrSol1[ez][ey][ex][slot_omega2_y] = arrOmega2[ez][ey][ex][slot_omega2_y];
          arrSol1[ez][ey][ex][slot_omega2_z] = arrOmega2[ez][ey][ex][slot_omega2_z];
...
PetscErrorCode DUAL_MAC::assemble_sol2_from_components(DM dmSol_2, Vec u2, Vec omega1, Vec sol2)
{
    PetscFunctionBeginUser;
    ...
    for (PetscInt ez = startz; ez < startz + nz; ++ez) {
      for (PetscInt ey = starty; ey < starty + ny; ++ey) {
        for (PetscInt ex = startx; ex < startx + nx; ++ex) {
          ...
          // 复制 omega1
          arrSol2[ez][ey][ex][slot_omega1_x] = arrOmega1[ez][ey][ex][slot_omega1_x];
          arrSol2[ez][ey][ex][slot_omega1_y] = arrOmega1[ez][ey][ex][slot_omega1_y];
          arrSol2[ez][ey][ex][slot_omega1_z] = arrOmega1[ez][ey][ex][slot_omega1_z];
