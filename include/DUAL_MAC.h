...
#define DUAL_MAC_DEBUG_LOG(...)                                                \
  do {                                                                         \
    PetscCall(PetscPrintf(PETSC_COMM_WORLD, __VA_ARGS__));                     \
  } while (0)
#else
#define DUAL_MAC_DEBUG_LOG(...)                                                \
  do {                                                                         \
...
public:
  ...
  // 带参数的构造函数
  DUAL_MAC(PetscReal time, PetscReal nu, PetscInt NX, PetscInt NY, PetscInt NZ,
           PetscInt Nt, PetscReal xmin, PetscReal xmax, PetscReal ymin,
  ...
  // 辅助函数：从组件组装完整解向量
  PetscErrorCode assemble_sol1_from_components(DM dmSol_1, Vec u1, Vec omega2,
                                               Vec sol1);
  PetscErrorCode assemble_sol2_from_components(DM dmSol_2, Vec u2, Vec omega1,
                                               Vec sol2);
...
private:
  ...
  PetscErrorCode apply_exterior_derivative_0_to_1(
      Vec v0_slot, Vec v1_slot); // d_0: 0形式 -> 1形式（顶点 -> 边）
  PetscErrorCode apply_exterior_derivative_1_to_2(
      Vec v1_slot, Vec v2_slot); // d_1: 1形式 -> 2形式（边 -> 面）
  PetscErrorCode apply_exterior_derivative_2_to_3(
      Vec v2_slot, Vec v3_slot); // d_2: 2形式 -> 3形式（面 -> 单元中心，仅3D）
  PetscErrorCode
  apply_hodge_star_1(Vec v1_slot, Vec v2_slot); // *: 1形式 -> 2形式（边 -> 面）
  PetscErrorCode
  apply_hodge_star_2(Vec v2_slot, Vec v1_slot); // *: 2形式 -> 1形式（面 -> 边）

  // 辅助函数：获取坐标数组
  PetscErrorCode get_coordinate_arrays(PetscScalar ***cArrX,
                                       PetscScalar ***cArrY,
                                       PetscScalar ***cArrZ = NULL);

  // 辅助函数：获取stencil location的slot索引
  PetscErrorCode get_location_slot(DMStagStencilLocation loc,
                                   PetscInt dof_index, PetscInt *slot);
  ...
  PetscErrorCode setup_initial_solution(RefSol refSol, Vec u1_0, Vec u2_0,
                                        Vec omega1_0, Vec omega2_0);
  ...
  PetscErrorCode assemble_omega1_u1_conv_rhs(DM dmRhs, Vec rhs, DM dmSol_2,
                                             Vec omega1_0, DM dmU1, Vec u1_0);
  PetscErrorCode assemble_omega2_curl_rhs(DM dmRhs, Vec rhs, DM dmOmega2,
                                          Vec omega2_0, PetscReal Re);
  PetscErrorCode extract_u1_from_solution(DM dmSolSrc, Vec sol, DM dmSolDst,
                                          Vec u1_half);
  PetscErrorCode compute_curl_u1_to_omega2(DM dmSol_1, Vec u1_half,
                                           Vec omega2_half);

  // 时间推进：1-形式与2-形式子步
  // time: 当前整数时间步 k（对于 time_evolve1，直接用于外力项 f^k；对于
  // time_evolve2，用于计算半整数时间步 k-1/2）
  PetscErrorCode time_evolve1(DM dmSol_1, DM dmSol_2, Vec sol1_old,
                              Vec sol1_new, Vec sol2_old, Vec sol2_new,
                              ExternalForce externalForce, PetscReal time);
  PetscErrorCode time_evolve2(DM dmSol_1, DM dmSol_2, Vec sol1_old,
                              Vec sol1_new, Vec sol2_old, Vec sol2_new,
                              ExternalForce externalForce, PetscReal time);
  // 初始化PETSc对象为NULL
  ...
  PetscErrorCode assemble_1form_system_matrix(DM dmSol_1, DM dmSol_2, Mat A,
                                              Vec rhs, Vec u1_prev,
                                              Vec omega1_known, Vec omega2_prev,
                                              ExternalForce externalForce,
                                              PetscReal time, PetscReal dt);

  // 组装右端项（不包括外力）
  // 包含时间导数项、对流项和旋度项
  PetscErrorCode assemble_rhs1_vector(DM dmSol_1, Vec rhs, Vec u1_prev,
  ...
  PetscErrorCode assemble_u1_conv_matrix(DM dmSol_1, Mat A, DM dmSol_2,
                                         Vec omega1_known);
  ...
  // 组装外力项向量
  PetscErrorCode assemble_force1_vector(DM dmSol_1, Vec rhs,
                                        ExternalForce externalForce,
                                        PetscReal time);

  // 组装robust外力项向量：R1(f)，在棱上做切向线积分平均
  PetscErrorCode assemble_robust_force1_vector(DM dmSol_1, Vec rhs,
                                               ExternalForce externalForce,
                                               PetscReal time);

  // ===== 2形式系统矩阵组装相关函数 =====
  // 组装完整的2形式系统矩阵和右端项（包括外力项）
  PetscErrorCode assemble_2form_system_matrix(DM dmSol_1, DM dmSol_2, Mat A,
                                              Vec rhs, Vec u2_prev,
                                              Vec omega2_known, Vec omega1_prev,
                                              ExternalForce externalForce,
                                              PetscReal time, PetscReal dt);

  // 组装右端项（不包括外力）
  // 包含时间导数项、对流项和旋度项
  PetscErrorCode assemble_rhs2_vector(DM dmSol_2, Vec rhs, Vec u2_prev,
                                      DM dmSol_1, Vec omega2_known,
                                      Vec omega1_prev, PetscReal dt);

  // 组装2形式速度时间导数矩阵：1/dt * I（对 u₂）
  PetscErrorCode assemble_u2_dt_matrix(DM dmSol_2, Mat A, PetscReal dt);

  // 组装对流项矩阵：0.5 * ω₂^{h,k-1/2} ×（对 u₂）
  PetscErrorCode assemble_u2_conv_matrix(DM dmSol_2, Mat A, DM dmSol_1,
                                         Vec omega2_known);
  ...
  // 组装外力项向量
  PetscErrorCode assemble_force2_vector(DM dmSol_2, Vec rhs,
                                        ExternalForce externalForce,
                                        PetscReal time);
  // 组装Pressure-robust外力项向量
  PetscErrorCode assemble_robust_force2_vector(DM dmSol_2, Vec rhs,
                                               ExternalForce externalForce,
                                               PetscReal time);
};

// 线性求解器接口（定义于 src/linearsolver.cpp）
PetscErrorCode solve_linear_system_basic(
    Mat A, Vec rhs, Vec sol, DM dm, const char *optionsPrefix = NULL,
    PetscBool attachPressureNullspace = PETSC_FALSE, PetscReal dt = 0.0,
    PetscReal alphaExternal = 1.0, PetscReal gammaExternal = 0.0);
PetscErrorCode
solve_linear_system_graddiv(Mat A, Vec rhs, Vec sol, DM dm, PetscReal gamma,
                            const char *optionsPrefix = NULL,
                            PetscBool attachPressureNullspace = PETSC_FALSE,
                            PetscReal dt = 0.0, PetscReal alphaExternal = 1.0,
                            PetscReal gammaExternal = 0.0);
