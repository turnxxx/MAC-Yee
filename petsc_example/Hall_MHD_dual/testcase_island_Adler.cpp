...
#define p0_(u, x, y, z) {                            \
    u = (1.0-_EPSILON_*_EPSILON_)/(2.0*pow(cosh(y/_DELTA_)+_EPSILON_*cos(x/_DELTA_),2));                                 \
}
...
#define Adelta_(A, x, y, z){  \
    A[0] = 0.0; \
    A[1] = 0.0; \
    A[2] = 2.0*_GAM_/M_PI/M_PI*cos(M_PI*x)*cos(0.5*M_PI*y);       \
}

#define A0_(A, x, y, z){  \
    A00_(A,x,y,z); \
    real_t Adelta[3]; \
    Adelta_(Adelta,x,y,z); \
    A[0] += Adelta[0]; \
    A[1] += Adelta[1]; \
    A[2] += Adelta[2]; \
}
...
#define Bdelta_(B, x, y, z){  \
    B[0] = - _GAM_/M_PI*cos(M_PI*x)*sin(0.5*M_PI*y); \
    B[1] = 2.0*_GAM_/M_PI*sin(M_PI*x)*cos(0.5*M_PI*y); \
    B[2] = 0.0; \
}
...
#define jdelta_(j, x, y, z){  \
    j[0] = 0.0; \
    j[1] = 0.0; \
    j[2] = 5.0/2.0*_GAM_*cos(M_PI*x)*cos(0.5*M_PI*y); \
}

#define j0_(j, x, y, z){  \
    j00_(j, x, y, z); \
    real_t jdelta[3]; \
    jdelta_(jdelta, x, y, z); \
    j[0] += jdelta[0]; \
    j[1] += jdelta[1]; \
    j[2] += jdelta[2]; \
}
...
#define fA_(f, t, x, y, z){ \
    f[0] = 0.0;                                 \
    f[1] = 0.0;                                 \
    f[2] = (_EPSILON_*_EPSILON_ - 1.0)/(Rm*_DELTA_*pow(cosh(y/_DELTA_) + _EPSILON_*cos(x/_DELTA_),2));   \
}
...
ProblemData *GetIslandAdlerProblemData(ParamList param)
{
    ...
    // x-periodic
    Array<int> ess_bdr_normal({0,1,0,1,0,0});
    Array<int> ess_bdr_magnetic({0,1,0,1,0,0});
