...
#define UMAG (1.0/M_PI)
#define PMAG (1.0/M_PI)
#define BMAG (2.0)
...
#define gradu_(u, t, x, y, z) {                           \
    u[0] = 0.0; \
    u[1] = -(sin(y))*F1_(t)*UMAG; \
    u[2] = 0.0; \
    u[3] = 0.0; \
    u[4] = 0.0; \
    u[5] = (cos(z))*F2_(t)*UMAG; \
    u[6] = (cos(x))*F3_(t)*UMAG; \
    u[7] = 0.0; \
    u[8] = 0.0; \
}
...
#define p_(u, t, x, y, z) {                            \
    u = sin(2*M_PI*x)*sin(2*M_PI*y)*sin(2*M_PI*z)*t*PMAG;                                 \
}

#define p0_(u, x, y, z) {                            \
    u = sin(2*M_PI*x)*sin(2*M_PI*y)*sin(2*M_PI*z)*0.0*PMAG;                                 \
}

#define gradp_(u, t, x, y, z) {                            \
    u[0] = 2.*M_PI*cos(2*M_PI*x)*sin(2*M_PI*y)*sin(2*M_PI*z)*(t)*PMAG;                                 \
    u[1] = 2.*M_PI*sin(2*M_PI*x)*cos(2*M_PI*y)*sin(2*M_PI*z)*(t)*PMAG;                                 \
    u[2] = 2.*M_PI*sin(2*M_PI*x)*sin(2*M_PI*y)*cos(2*M_PI*z)*(t)*PMAG;                                 \
}

#define A_(A, t, x, y, z){  \
    A[0] = (cos(y))*F1_(t)*BMAG; \
    A[1] = (sin(z))*F2_(t)*BMAG; \
    A[2] = (sin(x))*F3_(t)*BMAG; \
}
...
#define At_(A, t, x, y, z){  \
    A[0] = (cos(y))*F1DT_(t)*BMAG; \
    A[1] = (sin(z))*F2DT_(t)*BMAG; \
    A[2] = (sin(x))*F3DT_(t)*BMAG; \
}

#define B_(B, t, x, y, z){  \
    B[0] = -(cos(z))*F2_(t)*BMAG; \
    B[1] = -(cos(x))*F3_(t)*BMAG; \
    B[2] = (sin(y))*F1_(t)*BMAG; \
}
...
#define j_(j, t, x, y, z){  \
    j[0] = (cos(y))*F1_(t)*BMAG; \
    j[1] = (sin(z))*F2_(t)*BMAG; \
    j[2] = (sin(x))*F3_(t)*BMAG; \
}

#define curlj_(j, t, x, y, z){  \
    j[0] = -(cos(z))*F2_(t)*BMAG; \
    j[1] = -(cos(x))*F3_(t)*BMAG; \
    j[2] = (sin(y))*F1_(t)*BMAG; \
}
