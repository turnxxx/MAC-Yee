...
#define A0_(A, x, y, z){  \
    A[0] = BB_*(-cos(z)+cos(y)); \
    A[1] = BB_*(-cos(x)+cos(z)); \
    A[2] = BB_*(cos(2.0*y)+2.0*cos(x)); \
}

#define B0_(B, x, y, z){  \
    B[0] = BB_*(-2.0*sin(2.0*y)+sin(z)); \
    B[1] = BB_*(2.0*sin(x)+sin(z)); \
    B[2] = BB_*(sin(x)+sin(y)); \
}
...
#define j0_(j, x, y, z){  \
    j[0] = BB_*(cos(y)-cos(z)); \
    j[1] = BB_*(-cos(x)+cos(z)); \
    j[2] = BB_*(cos(y*2.0)*4.0+cos(x)*2.0); \
}
