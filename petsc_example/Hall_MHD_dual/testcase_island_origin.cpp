...
#define A0_(A, x, y, z){  \
    A[0] = 0.0; \
    A[1] = 0.0; \
    A[2] = _DELTA_*log(cosh(y/_DELTA_)+_EPSILON_*cos(x/_DELTA_))+_GAM_*cos(M_PI*x)*cos(0.5*M_PI*y);       \
}

#define B0_(B, x, y, z){  \
    B[0] = sinh(y/_DELTA_)/(cosh(y/_DELTA_)+_EPSILON_*cos(x/_DELTA_)) \
         - 0.5*M_PI*_GAM_*cos(M_PI*x)*sin(0.5*M_PI*y); \
    B[1] = _EPSILON_*sin(x/_DELTA_)/(cosh(y/_DELTA_)+_EPSILON_*cos(x/_DELTA_))  \
    + M_PI*_GAM_*sin(M_PI*x)*cos(0.5*M_PI*y); \
    B[2] = 0.0; \
}
...
#define j0_(j, x, y, z){  \
    ...
    +5.0/4.0*M_PI*M_PI*_GAM_*cos(M_PI*x)*cos(0.5*M_PI*y); \
}
...
/*
f[2] = 1.0*(1.0-_EPSILON_*_EPSILON_)/(Rm*_DELTA_*pow(cosh(y/_DELTA_)+_EPSILON_*cos(x/_DELTA_),2));   \
*/
