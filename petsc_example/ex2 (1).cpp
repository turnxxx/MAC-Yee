...
static PetscErrorCode output_matrix(Mat &pA, const char *name)
{
...
int main(int argc, char **argv)
{
  ...
  /* Create 2D DMStag for the solution, and set up.用括号括起来是限制了dof0,dof1,dof2和
  stencilWidth的作用域，没有什么特别意义 */
  {
    const PetscInt dof0 = 0, dof1 = 1, dof2 = 1; /* 1 dof on each edge and element center */
    const PetscInt stencilWidth = 1;
    PetscCall(DMStagCreate2d(PETSC_COMM_WORLD,
                             DM_BOUNDARY_NONE,
                             DM_BOUNDARY_NONE, 3, 3, PETSC_DECIDE, PETSC_DECIDE,
                             dof0, dof1, dof2, DMSTAG_STENCIL_BOX, stencilWidth, NULL, NULL, &dmSol));
  ...
  PetscCall(output_matrix(A, "ex2_A.mat"));
  /* Check Solution */
...
static PetscErrorCode CreateSystem(DM dmSol, Mat *pA, Vec *pRhs, PetscBool pinPressure)
{
  ...
  PetscCall(DMStagGetGlobalSizes(dmSol, &N[0], &N[1], NULL)); // 获得全局
  hx = 1.0 / N[0];
  hy = 1.0 / N[1];
  ...
  { /* With DMStag, always iterate x fastest, y second fastest, z slowest */
    for (ex = startx; ex < startx + nx; ++ex)
    {
      ...
      { /* Pin the first pressure node, if requested */
        DMStagStencil row;
...
static PetscErrorCode AttachNullspace(DM dmSol, Mat A)
{
  ...
  PetscFunctionBeginUser;
  // 创见一个与dmSol兼容的DMStag对象dmPressure，但只有一个单元自由度,从而确保了只有压力向量
  PetscCall(DMStagCreateCompatibleDMStag(dmSol, 0, 0, 1, 0, &dmPressure));
  // 创建一个常量向量constantPressure,并设置为1.0
  ...
  // 创建一个与dmSol兼容的向量basis
  PetscCall(DMCreateGlobalVector(dmSol, &basis));
  // 将constantPressure迁移到basis,basis是一个与dmSol兼容的向量，即包含了dmSol的全部自由度
  // DMStagMigrateVec函数将constantPressure迁移到basis,即让basis中那些在单元中心的自由度
  // 与constantPressure的值相同，其他自由度为0
  PetscCall(DMStagMigrateVec(dmPressure, constantPressure, dmSol, basis));
  // 创建一个压力零空间matNullSpace
  PetscCall(MatNullSpaceCreate(PetscObjectComm((PetscObject)dmSol),
                               PETSC_FALSE, 1, &basis, &matNullSpace));
  // 销毁basis和constantPressure
...
static PetscErrorCode CreateReferenceSolution(DM dmSol, Vec *pSolRef)
{
  ...
  for (ey = starty; ey < starty + ny + nExtra[1]; ++ey)
  {
    for (ex = startx; ex < startx + nx + nExtra[0]; ++ex)
    {
      ...
      { /* Don't fill on the dummy elements (though you could, and these values would just be ignored) */
